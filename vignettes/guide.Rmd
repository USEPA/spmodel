...
\begin{equation*}
  \mathbf{R} = [(\mathbf{I} - \phi \mathbf{W})(\mathbf{I} - \phi \mathbf{W})^\intercal]^{-1},
\end{equation*}
...
```{r log_zn, fig.cap="Distribution of log zinc concentration in the moss data.", out.width = "90%", fig.align="center", echo = FALSE}
...
```{r log_abund, fig.cap="Distribution of log abundance in the seal data. Polygons are grey where abundance data were not collected.", out.width = "85%", fig.align="center", echo = FALSE}
...
```{r}
spamod <- spautor(log_abund ~ 1, seal, spcov_type = "car")
```
...
```{r}
augment(spamod, newdata = spamod$newdata)
```
Notice that when calling `augment()` for prediction data originally treated as missing data, the `newdata` argument is required and is the `newdata` object in the fitted model object (`spamod`).
