...
$$ 
where $\mathbf{M}$ is a matrix of Euclidean distances among observations. Recall that $\phi$ is the range parameter, controlling the behavior of the covariance function as a function of distance. Parameterizations for `splm()` spatial covariance types and their $\mathbf{R}$ matrices can be seen by running `help("splm", "spmodel")` or in the [`splm()` Spatial Covariance Functions](https://usepa.github.io/spmodel/articles/technical.html#sec:splm-fn) section of the Technical Details vignette. Some of these spatial covariance types (e.g., Matérn) depend on an extra parameter beyond $\sigma^2_{de}$, $\sigma^2_{ie}$, and $\phi$.
...
$$
where $\mathbf{W}$ is a weight matrix describing the neighborhood structure in $\mathbf{y}$. Parameterizations for `spautor()` spatial covariance types and their $\mathbf{R}$ matrices can be seen by running `help("spautor", "spmodel")` or in the [`spautor()` Spatial Covariance Functions](https://usepa.github.io/spmodel/articles/technical.html#sec:spautor-fn) section of the Technical Details vignette.
...
moss
```
We can learn more about `moss` by running `help("moss", "spmodel")`, and we can visualize the distribution of log zinc concentration in `moss` by running
...
The fixed effects coefficient table contains estimates, standard errors, degrees of freedom, t-statistics, and p-values for each fixed effect (here, the sample size is small enough that Satterthwaite denominator degrees of freedom are used by default; see the [`satterthwaite()`](https://usepa.github.io/spmodel/articles/technical.html#sec:satterthwaite-lm) section of the Technical Details vignette). From this table, we notice there is evidence that mean log zinc concentration significantly decreases with distance from the haul road (p-value < 2e-16). We see the fixed effect estimates by running
```{r}
coef(spmod)
```
...
Suppose we want to quantify the difference in model quality between the spatial model and a non-spatial model using the AIC and AICc criteria. We fit a non-spatial model in `spmodel` by running
```{r}
...
AICc(spmod, lmod)
```
The noticeably lower AIC and AICc of the spatial model indicate that it is a better fit to the data than the non-spatial model. Recall that these AIC and AICc comparisons are valid because both models are fit using restricted maximum likelihood (the default). Similarly, one could use the BIC [@schwarz1978estimating] as a model comparison statistic based on the likelihood.
...
tidy(spmod)
```
This tibble format makes it easy to pull out the coefficient names, estimates, standard errors, degrees of freedom, t-statistics, and p-values from the `summary()` output.
...
We can visualize the distribution of log seal trends in the `seal` data by running
...
We can visualize the model predictions by running
```{r, eval = FALSE}
...
For areal data models fit with `spautor()`, predictions cannot be computed at locations that were not incorporated in the neighborhood structure used to fit the model. Thus, predictions are only possible for observations in `data` whose response values are missing (`NA`), as their locations are incorporated into the neighborhood structure. For example, we make predictions of log seal trends at the missing polygons by running
```{r, eval = FALSE}
predict(sealmod)
```
...
`spmodel` offers several advanced features for fitting spatial linear models. We briefly discuss some of these features next using the `moss` data, the `moose` data, and some simulated data. For technical details regarding each advanced feature, see the Technical Details vignette.
...
We can visualize the simulated data by running
...
which assigns observations to index groups based on k-means clustering [@macqueen1967classification], ensures each index group has approximately 100 observations, uses the theoretically-correct variance adjustment, and does not use parallel processing. 
```{r}
...
Instead of using `local = TRUE`, we can explicitly set `local`. For example, we can fit a model using random assignment with 60 groups, use the pooled variance adjustment, and use parallel processing with two cores by running
```{r}
...
Likelihood-based statistics like `AIC()`, `AICc()`, `BIC()`, `logLik()`, and `deviance()` should not be used to compare a model fit with a big data approximation to a model fit without a big data approximation.
...
Poisson regression assumes the mean and variance are equal. Often in practice, the variance is larger than the mean. When the variance is larger than the mean, there is said to be overdispersion in the data. Negative binomial regression accounts for overdispersion via an extra dispersion parameter that allows the variance to change separately from the mean (the full negative binomial regression parameterization is available in the [Negative Binomial Distribution](https://usepa.github.io/spmodel/articles/technical.html#sec:nbinomial) section of the Technical Details vignette). We fit a spatial negative binomial regression model to the moose count data and make predictions by running
```{r, eval = FALSE}
...
The purpose of this example is to show more applications of the `splm()` and `spautor()` functions when the data are not an `sf` object, as well as to show a few other functions in `spmodel`. The `caribou` data are a designed experiment with two treatments observed on an equally spaced grid and can be analyzed as point-referenced or areal data. We view the first few rows of `caribou` by running
```{r}
caribou
```
...
```
There is significant evidence that at least one tarp cover impacts nitrogen. Note that, like in `summary()`, these p-values are associated with a hypothesis test that uses Satterthwaite denominator degrees of freedom by default when the sample size is small (here, an $F$-test; see the [Hypothesis Testing](https://usepa.github.io/spmodel/articles/technical.html#sec:testing-lm) section of the Technical Details vignette), and an asymptotic Chi-squared test otherwise.
