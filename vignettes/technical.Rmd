...
Additional notation is used in Section$~$\ref{sec:predict-lm} (`predict()`):
\begin{equation*}
  \begin{split}
...
The `AIC()` and `AICc()` functions in `spmodel` are defined for restricted maximum likelihood and maximum likelihood estimation, which maximize a likelihood. The AIC and AICc as defined by @hoeting2006model are given by 
...
\end{equation}
where $\mathbf{e}_p$ are the Pearson  and $diag(\mathbf{H}_s)$ is the diagonal of the spatial hat matrix, $\mathbf{H}_s \equiv \mathbf{X}^* (\mathbf{X}^{* \top} \mathbf{X}^*)^{-1} \mathbf{X}^{* \top}$ [@montgomery2021introduction]. The larger the Cook's distance, the larger the influence.
...
These formulas are analogous to the formulas used to obtain linear unbiased predictions of unobserved data (Equation$~$\ref{eq:blup}) and prediction variances (Equation$~$\ref{eq:blup_cov}) in Section$~$\ref{sec:predict-lm}. Model fits are evaluated using mean squared prediction error (mspe), formally defined as
\begin{equation*}
...
Options for big data leave-one-out cross validation rely on the `local` argument, which is passed to `predict()`. The `local` list for `predict()` is explained in detail in Section$~$\ref{sec:predict-lm}, but we provide a short summary of how `local` interacts with `loocv()` here. 
...
`AIC()` and `AICc()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:aic-lm}).

## `anova()` {#sec:anova-glm}

`anova()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:anova-lm}).

## `coef()` {#sec:coef-glm}

`coef()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:coef-lm}).

## `confint()` {#sec:confint-glm}

`confint()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:confint-lm}).

## `cooks.distance()`  {#sec:cooks-glm}

The Cook's distance is obtained by finding the standard generalized linear model Cook's distance after conditioning on $\mathbf{w}$. That is, after conditioning on $\mathbf{w}$ the Cook's distance is
\begin{equation}
\frac{\mathbf{e}_p^2}{p}\frac{diag(\mathbf{H}_c)}{1 - diag(\mathbf{H}_c)},
\end{equation}
where $\mathbf{e}_p^2$ are the Pearson residuals, $diag(\mathbf{H}_c)$ is the diagonal of the hat matrix conditional on $\mathbf{w}$ given by $\mathbf{H}_c = \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$ is the leverage (hat) matrix conditional on $\mathbf{w}$, $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$, and $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(\text{w}_i)$.

## `deviance()` {#sec:deviance-glm}

The deviance returned by `spmodel` is the deviance of the fitted model conditional on $\mathbf{w}$ (i.e., the deviance of the conditional model). This deviance is derived by taking the deviance definitions from Section$~$\ref{sec:distributions-glm} and evaluating $\mu_i$ at $w_i$.

## `fitted()` {#sec:fitted-glm}

The fitted values for the response on the link scale (`type = "link"`) are given by $\mathbf{w}$. The fitted values for the response on the response scale (`type = "response"`) are given by $g^{-1}(\mathbf{w})$. The fitted values for the spatial random errors (`type = "spcov"`) and random effects (`type = "randcov"`) are derived similarly as for spatial linear models (Section$~$\ref{sec:fitted-glm}) but treat $\mathbf{w}$ as the response instead of $\mathbf{y}$.

## `hatvalues()` {#sec:hatvalues-glm}

The leverage (hat) matrix is obtained by finding the standard generalized linear model leverage (hat) matrix after conditioning on $\mathbf{w}$. That is, after conditioning on $\mathbf{w}$ the leverage (hat) matrix, $\mathbf{H}_c$, is 
\begin{equation}
  \mathbf{H}_c = \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top,
\end{equation}
where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$ and $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(\text{w}_i)$.

## `logLik()` {#sec:loglik-glm}

`logLik()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:loglik-lm}).

## `loocv()` {#sec:loocv-glm}

`loocv()` for spatial generalized linear models are defined similarly as for spatial linear models (Section$~$\ref{sec:loocv-lm}), except that $\mathbf{w}$ is predicted instead of $\mathbf{y}$. Additionally, $\mathbf{H}_{-i}$ is determined from $\mathbf{H}$ using Helmert-Wolf blocking as is done for $\boldsymbol{\Sigma}_{-i}$ and $\boldsymbol{\Sigma}$.

### Big Data {#sec:bigdata-loocv-glm}

`loocv()` for big data spatial generalized linear models are defined similarly as for big data spatial linear models (Section$~$\ref{sec:bigdata-loocv-lm}), except that $\mathbf{w}$ is predicted instead of $\mathbf{y}$. Additionally, $\mathbf{H}_{l, l}$ is determined from each local neighborhood as is done for $\boldsymbol{\Sigma}_{l, l}$.

## `predict()` {#sec:predict-glm}

### `interval = "none"` {#sec:predict-none-glm}

Building from the empirical best linear unbiased predictions (i.e., empirical Kriging predictions) defined in Section$~$\ref{sec:predict-none-lm}, predictions of $\mathbf{w}_u$ are given on the link scale (`type = "link"`) by
\begin{equation}\label{eq:glm_blup}
...
\end{equation}
These predictions are given on the response (inverse link) scale (`type = "response"`) as $g^{-1}(\mathbf{\dot{w}}_u)$.

Similar to the covariance matrix of $\hat{\boldsymbol{\beta}}$, the covariance matrix of $\mathbf{\dot{w}}_u$ requires an adjustment to account for the fact that the $\mathbf{w}$ are not actually observed. First let $\boldsymbol{\Lambda} = \mathbf{X}_u^\top \mathbf{B} + \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}_o^{-1} + \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X} \mathbf{B}$ and $\mathbf{B} = (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \hat{\boldsymbol{\Sigma}}_o^{-1}$ and note that $\mathbf{\dot{w}}_u = \Lambda \hat{\mathbf{w}}_o$. Using the law of conditional variance and conditioning on $\mathbf{w}_o$ as if we had observed them, it follows that
\begin{equation*}
...
\end{equation*}
We assume $\hat{\mathbf{w}}_o$ is unbiased for $\mathbf{w}_o$ (i.e., $\text{E}(\hat{\mathbf{w}}_o | \mathbf{w}_o) = \mathbf{w}_o$). Then $\text{Cov}_{\mathbf{w}_o}[\text{E}(\Lambda \hat{\mathbf{w}}_o - \mathbf{w}_u | \mathbf{w}_o)] = \text{Cov}_{\mathbf{w}_o}(\Lambda \mathbf{w}_o - \mathbf{w}_u) = \hat{\boldsymbol{\Sigma}}_u - \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}^{-1}_o \hat{\boldsymbol{\Sigma}}^\top_{uo} + \mathbf{Q}(\mathbf{X}_o^\top \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X}_o)^{-1}\mathbf{Q}^\top$, where $\mathbf{Q} = \mathbf{X}_u - \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}^{-1}_o \mathbf{X}_o$, the usual form for the mean squared prediction error of $\hat{\mathbf{w}}_u$. Next note that (viewing $\mathbf{w}_u$ as a constant) $\text{E}_{\mathbf{w}_o}[\text{Cov}(\Lambda \hat{\mathbf{w}}_o - \mathbf{w}_u | \mathbf{w}_o)]$ can be viewed as the observed Fisher Information inverse, $- \mathbf{H}^{-1}$. Evaluating $\mathbf{H}$ at $\mathbf{a}$ eventually yields the adjusted covariance matrix of $\Lambda \hat{\mathbf{w}}_o - \mathbf{w}_u$ (`var_correct = TRUE`) given by
\begin{equation}\label{eq:glm_blup_cov}
  \text{Cov}(\Lambda \hat{\mathbf{w}}_o - \mathbf{w}_u) = \dot{\boldsymbol{\Sigma}}_u = \Lambda (-\mathbf{H}_\mathbf{a})^{-1} \Lambda^\top + \hat{\boldsymbol{\Sigma}}_u - \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}^{-1}_o \hat{\boldsymbol{\Sigma}}^\top_{uo} + \mathbf{Q}(\mathbf{X}_o^\top \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X}_o)^{-1}\mathbf{Q}^\top .
\end{equation}

The unadjusted covariance matrix of $\Lambda \hat{\mathbf{w}}_o - \mathbf{w}_u$ (`var_correct = FALSE`) can also be returned and is given by
...
When `se.fit = TRUE`, standard errors are returned on the link scale by taking the square root of the diagonal of $\dot{\boldsymbol{\Sigma}}_u$ in Equation$~$\ref{eq:glm_blup_cov} (`var_correct = TRUE`) or Equation$~$\ref{eq:glm_blup_cov_unadj} (`var_correct = FALSE`).

### `interval = "prediction"` {#sec:predict-predict-glm}

Predictions of $\mathbf{w}_u$ are returned on the link scale (`type = "link"`) by evaluating Equation$~$\ref{eq:glm_blup}. The (100 $\times$ `level`)% prediction interval for $(w_u)_i$ is $(\dot{w}_u)_i \pm z^* \sqrt{(\dot{\boldsymbol{\Sigma}}_u)_{i, i}}$, where $\sqrt{(\dot{\boldsymbol{\Sigma}}_u)_{i, i}}$ is the standard error of $(\dot{w}_u)_i$ obtained from `se.fit = TRUE`, $\Phi(z^*) = 1 - \alpha / 2$, $\Phi(\cdot)$ is the standard normal (Gaussian) cumulative distribution function, $\alpha = 1 -$ `level`, and `level` is an argument to `predict()`. The default for `level` is 0.95, which corresponds to a $z^*$ of approximately 1.96. These predictions and corresponding prediction intervals are returned on the response scale (`type = "response"`) by applying $g^{-1}(\cdot)$ (inverse link) to $(\dot{w_u})_i$ (the prediction), $(\dot{w}_u)_i - z^* \sqrt{(\dot{\boldsymbol{\Sigma}}_u)_{i, i}}$ (the prediction interval lower bound), and $(\dot{w}_u)_i + z^* \sqrt{(\dot{\boldsymbol{\Sigma}}_u)_{i, i}}$ (the prediction interval upper bound). Note that the prediction intervals are symmetric on the link scale but are not generally symmetric on the response scale. One could obtain symmetric prediction intervals on the response scale using the delta method.

### `interval = "confidence"` {#sec:predict-conf-glm}

Estimates for $(\mathbf{X}_u)_i \boldsymbol{\beta}$ (the fixed effects portion of the model) are returned on the link scale (`type = "link"`) by evaluating $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}}$ (i.e., fitted values corresponding to $(\mathbf{X}_u)_i)$. The (100 $\times$ `level`)% confidence interval for $(\mathbf{X}_u)_i \boldsymbol{\beta}$ is $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}} \pm z^* \sqrt{(\mathbf{X}_u)_i [\mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1}_o \mathbf{X})^{-1}](\mathbf{X}_u)_i^\top}$, where $(\mathbf{X}_u)_i$ is the $i$th row of $\mathbf{X}_u$, $\mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1}$ is the standard error of $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}}$ obtained from `se.fit = TRUE`, $\Phi(z^*) = 1 - \alpha / 2$, $\Phi(\cdot)$ is the standard normal (Gaussian) cumulative distribution function, $\alpha = 1 -$ `level`, and `level` is an argument to `predict()`. The default for `level` is 0.95, which corresponds to a $z^*$ of approximately 1.96. These estimates and corresponding confidence intervals are returned on the response scale (`type = "response"`) by applying $g^{-1}(\cdot)$ (inverse link) to $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}}$ (the estimate), $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}} - z^* \sqrt{\mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1}_o \mathbf{X})^{-1}}$ (the confidence interval lower bound), and $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}} + z^* \sqrt{\mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1}_o \mathbf{X})^{-1}}$ (the confidence interval upper bound). Note that the confidence intervals are symmetric on the link scale but are generally not symmetric on the response scale. One could obtain symmetric confidence intervals on the response scale using the delta method.

### `spautor()` extra steps {#sec:predict-spautor-glm}

The extra step required to obtain $\hat{\boldsymbol{\Sigma}}^{-1}_o$, $\hat{\boldsymbol{\Sigma}}_u$, and $\hat{\boldsymbol{\Sigma}}_{uo}$ is the same for spatial generalized autoregressive models as it is for spatial autoregressive models (Section$~$\ref{sec:predict-spautor-lm}).

### Big Data {#sec:bigdata-predict-glm}

`predict()` for big data spatial generalized linear models are defined similarly as for big data spatial linear models (Section$~$\ref{sec:bigdata-predict-glm}), except that $\mathbf{w}$ are subset or predicted (instead of $\mathbf{y}$) to find $\check{\mathbf{w}}_o$ (instead of $\check{\mathbf{y}}_o$). It standard errors are required, $\check{\mathbf{H}}_o$ is also found.

## `pseudoR2()` {#sec:pr2-glm}

`pseudoR2()` for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:pr2-lm}).

## `residuals()` {#sec:residuals-glm}

Residuals are obtained by applying standard generalized linear model definitions after conditioning on $\mathbf{w}$.

When `type = "response"`, response residuals are returned:
\begin{equation*}
 \mathbf{e}_{r} = \mathbf{y} - \mathbf{w}.
\end{equation*}
...
\end{equation*}
where $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(\text{w}_i)$.

When `type = "deviance"`, deviance residuals are returned:
\begin{equation*}
 \mathbf{e}_{d} = \text{sign}(\mathbf{e}_r) \sqrt{\text{deviance}_i},
\end{equation*}
where $\text{deviance}_i$ is the deviance of $\text{y}_i$ (conditional on $\text{w}_i$).

When `type = "standardized"`, standardized residuals are returned:
\begin{equation*}
 \mathbf{e}_{s} = \frac{\mathbf{e}_{d}}{\sqrt{1 - diag(\mathbf{H}_c)}},
\end{equation*}
where $diag(\mathbf{H}_c)$ is the diagonal of the hat matrix conditional on $\mathbf{w}$ given by $\mathbf{H}_c \equiv \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$, where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$.

For more on the motivation of residuals in `spmodel`, see Section$~$\ref{sec:residuals-lm}.
...
Covariance functions for `spgautor()` are defined the same as covariance functions for `spautor()` (Section$~$\ref{sec:spautor-fn}).

### `spglm()` Spatial Covariance Functions {#sec:spglm-glm}

Covariance functions for `spglm()` are defined the same as covariance functions for `splm()` (Section$~$\ref{sec:splm-fn}).

### Model-fitting {#sec:estimation-glm}

For spatial linear models, it is relatively straightforward to marginalize over $\boldsymbol{\beta}$ and the random components to obtain a distribution of only the data ($\mathbf{y}$) and covariance parameters ($\boldsymbol{\theta}$) -- this is the REML likelihood (Section$~$\ref{sec:estimation-lm}). For spatial generalized linear models, this is more challenging. First define $\mathbf{w} = \mathbf{X} \boldsymbol{\beta} + \boldsymbol{\tau} + \boldsymbol{\epsilon}$. Note that $\mathbf{w}$ is a latent effect that is not actually observed. Our goal is to marginalize the joint distribution of $\mathbf{y}$ and $\mathbf{w}$ over $\mathbf{w}$ to obtain a distribution of only the data ($\mathbf{y}$), a dispersion parameter ($\phi$), and covariance parameters ($\boldsymbol{\theta}$). To accomplish this feat, we use the Laplace approximation. We briefly describe this approach next, but it is described in full detail in @ver2023marginal. 

The marginal distribution of interest can be written hierarchically as
\begin{equation*}
  [\mathbf{y}|\mathbf{X}, \phi, \boldsymbol{\theta}] = \int_w [\mathbf{y} | g^{-1}(\mathbf{w}), \phi] [\mathbf{w} | \mathbf{X}, \boldsymbol{\theta}] d\mathbf{w} .
\end{equation*}
The term $[\mathbf{y}|g^{-1}(\mathbf{w}), \phi]$ is likelihood of the generalized linear model with mean function $g^{-1}(\mathbf{w})$, and the term $[\mathbf{w}|\mathbf{X}, \boldsymbol{\theta}]$ is the restricted log likelihood for $\mathbf{w}$ given the covariance parameters. 

Next define $\ell_\mathbf{w} = \ln([\mathbf{y} | g^{-1}(\mathbf{w}), \phi] [\mathbf{w} | \mathbf{X}, \boldsymbol{\theta}])$. Let $\mathbf{g}$ be the gradient vector where $g_i = \frac{\partial \ell_\mathbf{w}}{\partial w_i}$ and $\mathbf{H}$ be the Hessian matrix with $ij$th element $H_{i, j} = \frac{\partial^2 \ell_\mathbf{w}}{\partial w_i \partial w_j}$ (note that this use of $\mathbf{H}$ is not to be confused with the leverage (hat) matrix used elsewhere throughout this document). Using a multivariate Taylor series expansion around some point $\mathbf{a}$,
\begin{equation*}
  \int_\mathbf{w} \exp(\ell_\mathbf{w}) d\mathbf{w} \approx \int_\mathbf{w} \exp(\ell_\mathbf{a} + \mathbf{g}^\top(\mathbf{w} - \mathbf{a}) + \frac{1}{2}(\mathbf{w} - \mathbf{a})^\top \mathbf{H}_a (\mathbf{w} - \mathbf{a})) d\mathbf{w}.
\end{equation*}
If $\mathbf{a}$ is the value at which $\mathbf{g} = \mathbf{0}$, then 
\begin{equation*}
  \int_\mathbf{w} \exp(\ell_\mathbf{w}) d\mathbf{w} \approx exp(\ell_\mathbf{a}) \int_w \exp \left(-\frac{1}{2} (\mathbf{w} - \mathbf{a})^\top (- \mathbf{H})(\mathbf{w} - \mathbf{a}) \right) d\mathbf{w} = \exp(\ell_\mathbf{a}) (2 \pi)^{n/2} |-\mathbf{H}_a|^{-1/2}, 
\end{equation*}
where $\mathbf{H}_a$ is the Hessian evaluated at $\mathbf{a}$ and $|\cdot|$ is the determinant operator. The previous result follows from the normalizing constant of a Gaussian distribution with kernel $\exp(\frac{1}{2} (\mathbf{w} - \mathbf{a})^\top [(- \mathbf{H})^{-1}]^{-1}(\mathbf{w} - \mathbf{a}))$. Finally, we arrive at
\begin{equation*}
  \int_\mathbf{w} \exp(\ell_\mathbf{w}) d\mathbf{w} \approx [\mathbf{y} | g^{-1}(\mathbf{w}), \phi] [\mathbf{w} | \mathbf{X}, \boldsymbol{\theta}] (2 \pi)^{n/2} |-\mathbf{H}_a|^{-1/2}|,
\end{equation*}
which is a distribution that has been marginalized over $\mathbf{w}$ and depends only on the data, a dispersion parameter, and the covariance parameters. The approach we outlined to solve this integral is known as the Laplace approximation, and next we show how this likelihood is maximized.

Given $\mathbf{a}$, minus twice a profiled (by $\boldsymbol{\beta}$) marginal Laplace log-likelihood is given by
\begin{equation*}
  -2\ell_p(\phi, \boldsymbol{\theta}) = -2\ln([\mathbf{y} | g^{-1}(\mathbf{a}), \phi]) -2\ln([\mathbf{a} | \mathbf{X}, \boldsymbol{\theta}]) + \ln(|- \mathbf{H}_{\mathbf{a}}|).
\end{equation*}
Note that $\ln[\mathbf{a} | \mathbf{X}, \boldsymbol{\theta}]$ are the ML or REML log-likelihood equations from Equation$~$\ref{eq:ml-lik} and Equation$~$\ref{eq:reml-lik}, respectively, where now $\tilde{\boldsymbol{\beta}} = (\mathbf{X}^\top \mathbf{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \mathbf{\Sigma}^{-1} \mathbf{a}$.

Assuming $\mathbf{y}$ and $g^{-1}(\mathbf{w})$ are conditionally independent and $\phi$ and $\boldsymbol{\theta}$ are known, it can be shown that maximizing $\ell_{\mathbf{w}}$ (with respect to $\mathbf{w}$) amounts to maximizing (up to a constant)
\begin{equation*}
  \sum_{i = 1}^N \ln[y_i | w_i, \phi] - \frac{1}{2}(\mathbf{w} - \mathbf{X} \hat{\boldsymbol{\beta}})^\top \boldsymbol{\Sigma}^{-1}(\mathbf{w} - \mathbf{X}\hat{\boldsymbol{\beta}}).
\end{equation*}
Thus the gradient $\ell_{\mathbf{w}}$ can be shown to equal
\begin{equation*}
 \mathbf{g} = \mathbf{d} - \boldsymbol{\Sigma}^{-1}\mathbf{w} + \boldsymbol{\Sigma}^{-1}\mathbf{X} \hat{\boldsymbol{\beta}} = \mathbf{d} - \mathbf{P}\mathbf{w},
\end{equation*}
where $d_i = \partial \ln[y_i | g^{-1}(w_i), \phi]  / \partial w_i$ and $\mathbf{P} = \boldsymbol{\Sigma}^{-1} - \boldsymbol{\Sigma}^{-1} \mathbf{X} (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \boldsymbol{\Sigma}^{-1}$. And the Hessian of $\ell_{\mathbf{w}}$ is 
\begin{equation*}
  \mathbf{H} = \mathbf{D} - \mathbf{P},
\end{equation*}
where $D_{i, i} = \partial^2 \ln[y_i | g^{-1}(w_i), \phi]  / \partial w_i^2$. Note that $D_{i, j} = 0$ for $i \neq j$ because of conditional independence. 
...
\end{equation*}
where $0 < \alpha \leq 1$. Typically, $\alpha = 1$, but it can be lowered if the $\mathbf{g}$ is unstable. Generally, the Newton-Rhapson converges rapidly. The value of $\mathbf{w}$ at convergence is defined as $\mathbf{a}$.

It follows that finding $\phi$ and $\boldsymbol{\theta}$ for unknown $\mathbf{a}$ requires a doubly-iterative algorithm. First, a value of $\mathbf{a}$ is proposed (e.g., $\mathbf{a} = \mathbf{0}$). Then given $\mathbf{a}_0$, the Laplace log-likelihood is maximized, yielding $\hat{\phi}_0$ and $\hat{\boldsymbol{\theta}}_0$. Then given $\phi_0$ and $\boldsymbol{\theta}_0$, $\ell_{\mathbf{w}}$ is maximized, yielding $\mathbf{a}_1$. Then given $\mathbf{a}_1$, the Laplace log-likelihood is maximized, yielding $\hat{\phi}_1$ and $\hat{\boldsymbol{\theta}}_1$. Then given $\phi_1$ and $\boldsymbol{\theta}_1$, $\ell_{\mathbf{w}}$ is maximized, yielding $\mathbf{a}_2$. This process continues until convergence, yielding optimized values for $\phi$ and $\boldsymbol{\theta}$ and a value of $\mathbf{a}$ determined using these optimized values. 

Note that the Laplace approximation incorporates a likelihood, and as a result, the only estimation methods available via the `estmethod` argument are `"reml"` (the default) and `"ml"`. Fitting spatial generalized linear models requires a doubly-iterative algorithm, which makes it far more computationally expensive than fitting spatial linear models.

### Optimization {#sec:optim-glm}

Optimization for `spglm()` and `spgautor()` works as it does for `splm()` and `spautor()` (Section$~$\ref{sec:optim-lm}), with one additional step. The convergence criteria for $\mathbf{w}$ (within each covariance parameter iteration) is achieved when the largest absolute value of $\mathbf{w}_k - \mathbf{w}_{k - 1}$ is less than $10^{-4}$ or $k > 50$ ($k$ indexes the Newton-Rhapson iterations).

#### Grid Search {#sec:grid-glm}

The grid search for `spglm()` and `spgautor()` works as it does for `splm()` and `spautor()` (Section$~$\ref{sec:grid-lm}) For negative binomial, beta, gamma, and inverse Gaussian families, the initial value of the dispersion parameter is set to one. Note that for `spglm()` and `spgautor()`, the grid search initial values are on the link scale and the grid search sample variance is calculated regressing $\log(\mathbf{y} + 1)$ on $\mathbf{X}$ instead of $\mathbf{y}$ on $\mathbf{X}$.

### Hypothesis Testing {#sec:testing-glm}

Hypothesis testing for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:testing-lm}). That is, the hypothesis tests are asymptotic z-tests based on the normal (Gaussian) distribution (Wald tests). The null hypothesis for the test associated with each $\hat{\beta}_i$ is that $\beta_i = 0$. The spatial genearlized linear models, $\text{Cov}(\hat{\boldsymbol{\beta}})$ requires an adjustment to account for the fact that the $\mathbf{w}$ are not actually observed. First let $\mathbf{B} = (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \boldsymbol{\Sigma}^{-1}$ and note that $\hat{\boldsymbol{\beta}} = \mathbf{B}\mathbf{w}$. Using the law of conditional variance and conditioning on $\mathbf{w}$ as if we had observed them, it follows that 
\begin{equation*}
...
\end{equation*}
We assume $\hat{\mathbf{w}}$ is unbiased for $\mathbf{w}$ (i.e., $\text{E}(\hat{\mathbf{w}} | \mathbf{w}) = \mathbf{w}$). Then $\text{Cov}(\text{E}(\mathbf{B} \hat{\mathbf{w}} | \mathbf{w})) = \text{Cov}(\mathbf{B}\mathbf{w}) = \mathbf{B} \boldsymbol{\Sigma} \mathbf{B}^\top$, which reduces to 
$(\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1}$, the usual form for $\text{Cov}(\hat{\boldsymbol{\beta}})$. Next note that $\text{Cov}(\hat{\mathbf{w}} | \mathbf{w})$ can be viewed as the inverse of the observed Fisher Information, which is $-\mathbf{H}^{-1}$, which depends on $\mathbf{w}$ through the diagonal elements in $\mathbf{D}$. Evaluating $-\mathbf{H}$ at $\mathbf{a}$ yields the adjusted covariance matrix of $\hat{\boldsymbol{\beta}}$ given by
\begin{equation*}
  \text{Cov}(\hat{\boldsymbol{\beta}}) = \mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1} .
\end{equation*}

### Random Effects (`spglm()` only) {#sec:random-glm}

Random effects for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:random-glm}). Note that random effects for spatial generalized linear models are on the link scale.

### Anisotropy (`spglm()` only) {#sec:anisotropy-glm}

Anisotropy for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:anisotropy-glm}). Note that anisotropy parameters for spatial generalized linear models are on the link scale.

### Partition Factors {#sec:partition-glm}

Partition factors for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:partition-glm}).

### Big Data (`spglm()` only) {#sec:bigdata-glm}

Big data model-fitting for spatial generalized linear models in many ways the same as for spatial linear models (Section$~$\ref{sec:bigdata-lm}). The `local` argument behaves the same for spatial generalized linear models as it is for spatial linear models. This is because fundamentally, the "local indexing" approach to representing $\boldsymbol{\Sigma}$ blockwise is still applied and serves as the basis for massive computational gains when fitting spatial generalized linear models. 

The additional step is required to fit big data spatial generalized linear models involves efficiently manipulating the Hessian, $\mathbf{H}$, to obtain its inverse and log determinant. Before providing further details, we review the Sherman-Morrison-Woodbury (SMW) formula  [@sherman1949adjustment; @sherman1950adjustment; @woodbury1950inverting]. The SMW formula states that for an $n \times n$ matrix $\mathbf{A}$, an $n \times k$ matrix $\mathbf{U}$, a $k \times k$ matrix $\mathbf{C}$, and a $k \times n$ matrix $\mathbf{V}$,
\begin{equation*}
...
\end{equation*}
The determinant result above implies
\begin{equation*}
  \ln|\mathbf{A} + \mathbf{U} \mathbf{C} \mathbf{V}| = \ln|\mathbf{A}| + \ln|\mathbf{C}| + \ln|\mathbf{C}^{-1} + \mathbf{V}\mathbf{A}^{-1}\mathbf{U}|.
\end{equation*}
The SMW formula is important because if the inverse and log determinant of $\mathbf{A}$ is efficient to compute, then the inverse and log determinant of the desired sum can also be efficient to compute because except for $\mathbf{A}^{-1}$ and $\mathbf{|A|}$, this sum only depends on $k \times k$ inverses and log determinants.

Recall (Section$~$\ref{sec:estimation-glm}) that the Hessian can be written as
\begin{equation*}
  \mathbf{H} = \mathbf{D} - \boldsymbol{\Sigma}^{-1} + \boldsymbol{\Sigma}^{-1} \mathbf{X} (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1} \mathbf{X}^\top \boldsymbol{\Sigma}^{-1},
\end{equation*}
...
Fitting big data spatial generalized linear models requires a doubly-iterative algorithm, which makes it far more computationally expensive than fitting big data spatial linear models.
...
The corrected variance-covariance matrix of the fixed effects (`var_correct = TRUE`) is given by $\mathbf{B} (-\mathbf{H}_a)^{-1} \mathbf{B}^\top + (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1}$.
The uncorrected variance-covariance matrix (`var_correct = FALSE`) is given by
  $\text{Cov}(\hat{\boldsymbol{\beta}}) = (\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X})^{-1}$.


## Distribution Parameteriziations {#sec:distributions-glm}

Below we provide definitions associated with each of the six generalized linear models families available in `spmodel`. Note that the poisson, binomial, gamma, and inverse Gaussian distributions are members of the exponential family, and their deviance is typically expressed as twice the difference in log-likelihoods between the saturated and fitted model times the dispersion parameter. The negative binomial and beta distributions are members of the exponential family when the dispersion parameter ($\vartheta$) is known, and their deviance is typically expressed as just twice the difference in log-likelihoods between the saturated and fitted model. 
...
\end{equation*}
where $y$ is a non-negative integer, $\mu > 0$, $\text{E}(y) = \mu$, and $\text{Var}(y) = \mu$. 

The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu) = y \log(\mu) - \mu - \log(y!).
\end{equation*}

Using the inverse log link and writing in terms of $\mu = exp(w)$, the log-likelihood can be written as
\begin{equation*}
  \log f(y | \mu) = yw - \exp(w) - \log(y!).
\end{equation*}

The derivative of the Poisson distribution with respect to $w$ is
\begin{equation*}
  \frac{d}{d w} \log f(y | \mu) = y - \exp(w).
\end{equation*}

The second derivative of the Poisson distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \exp(w).
\end{equation*}

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = \sum_i y_i \log(y_i) - y_i - \log(y_i!).
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \sum_i y_i \log(\hat{\mu}_i) - \hat{\mu}_i - \log(y_i!).
\end{equation*}
...
\end{equation*}
where $y$ is a non-negative integer, $\mu > 0$, $\varphi > 0$, $\text{E}(y) = \mu$, and $\text{Var}(y) = \mu + \frac{\mu^2}{\varphi}$. 

The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu, \varphi) = \log(\Gamma(y + \varphi)) - \log(\Gamma(\varphi)) - \log(y!) + y \log(\mu) - y \log(\mu + \varphi) + \varphi \log(\varphi) - \varphi \log(\mu + \varphi).
\end{equation*}

Using the inverse log link and writing in terms of $\mu = exp(w)$, the log-likelihood can be written as
\begin{equation*}
  \log f(y | \mu) = \log(\Gamma(y + \varphi)) - \log(\Gamma(\varphi)) - \log(y!) + y [w - \log(\exp(w) + \varphi)] + \varphi [\log(\varphi) - \log(\exp(w) + \varphi)].
\end{equation*}

The derivative of the negative binomial distribution with respect to $w$ is
\begin{equation*}
\begin{split}
  \frac{d}{d w} \log f(y | \mu) & = y\left(1 - \frac{\exp(w)}{\exp(w) + \varphi}\right) + \varphi \left(- \frac{\exp(w)}{\exp(w) + \varphi} \right) \\ 
  & = \frac{y\varphi}{\exp(w) + \varphi} - \frac{\varphi \exp(w)}{\exp(w) + \varphi} \\
  & =  \frac{\varphi (y - \exp(w))}{\exp(w) + \varphi}.
\end{split}
\end{equation*}

The second derivative of the negative binomial distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \frac{\varphi \exp(w) (y + \varphi)}{(\varphi + \exp(w))^2}
\end{equation*}

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = \sum_i \log(\Gamma(y_i + \varphi)) - \log(\Gamma(\varphi)) - \log(y_i!) + y_i \log(y_i) - y_i \log(y_i + \varphi) + \varphi \log(\varphi) - \varphi \log(y_i + \varphi).
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \sum_i \log(\Gamma(y_i + \varphi)) - \log(\Gamma(\varphi)) - \log(y_i!) + y_i \log(\hat{\mu}_i) - y_i \log(\hat{\mu}_i + \varphi) + \varphi \log(\varphi) - \varphi \log(\hat{\mu}_i + \varphi).
\end{equation*}

Thus the deviance is
\begin{equation*}
  2 \sum_i y_i [\log(y_i) - \log(y_i + \varphi) - \log(\hat{\mu}_i) + \log(\hat{\mu}_i + \varphi)] + \varphi [ - \log(y_i + \varphi) + \log(\hat{\mu}_i + \varphi)].
\end{equation*}
...
\end{equation*}
where $m$ is the (known) number of Bernoulli trials, $y$ is a non-negative integer, $0 \le \mu \le 1$, $\text{E}(y) = m\mu$, and $\text{Var}(y) = m\mu (1 - \mu)$. 

The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu) = \log \left[ \binom{m}{y} \right] + y \log(\mu) + (m - y) \log(1 - \mu).
\end{equation*}

Using the inverse logit link and writing in terms of $\mu = \exp(w) / (1 + \exp(w))$, the log-likelihood can be written as
\begin{equation*}
  \log f(y | \mu) = \log \left[ \binom{m}{y} \right] + y \log(\exp(w) / (1 + \exp(w))) + (m - y) \log(1 - \exp(w) / (1 + \exp(w))).
\end{equation*}

The derivative of the binomial distribution with respect to $w$ is
\begin{equation*}
  \frac{d}{d w} \log f(y | \mu) = y - \frac{m \exp(w)}{1 + \exp(w)}
\end{equation*}

The second derivative of the binomial distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \frac{m \exp(w)}{(1 + exp(w))^2}.
\end{equation*}

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = \sum_i \log \left[ \binom{m_i}{y_i} \right] + y_i \log(y_i) + (m_i - y_i) \log(m_i - y_i).
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \sum_i \log \left[ \binom{m_i}{y_i} \right] + y_i \log(m_i\hat{\mu}_i) + (m_i - y_i) \log(m_i - m_i \hat{\mu}_i).
\end{equation*}

Thus the deviance is
\begin{equation*}
  2 \sum_i y_i[ \log(y_i) - \log(m_i \hat{\mu}_i)] + (m_i - y_i)[ \log(m_i - y_i) - \log(m_i - m_i \hat{\mu}_i) ]
\end{equation*}
...
\end{equation*}
where $0 < y < 1$, $0 < \mu < 1$, $\text{E}(y) = \mu$, and $\text{Var}(y) = \mu (1 - \mu) / (1 + \varphi)$. 

The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu) = \log(\Gamma(\varphi)) - \log(\Gamma(\mu \varphi)) - \log(\Gamma((1 - \mu)\varphi)) +  (\mu \varphi - 1) \log(y) + ((1 - \mu)\varphi - 1) \log(1 - y).
\end{equation*}

Using the inverse logit link and writing in terms of $\mu = exp(w)$, the log-likelihood can be written as
\begin{equation*}
\begin{split}
  \log f(y | \mu, \varphi) & = \log(\Gamma(\varphi)) - \log(\Gamma(\frac{\exp(w)}{1 + \exp(w)} \varphi)) - \log(\Gamma((1 - \frac{\exp(w)}{1 + \exp(w)})\varphi)) \\
  & +  (\frac{\exp(w)}{1 + \exp(w)} \varphi - 1) \log(y) + ((1 - \frac{\exp(w)}{1 + \exp(w)})\varphi - 1) \log(1 - y).
\end{split}
\end{equation*}

It can be shown that the derivative of beta distribution with respect to $w$ is
\begin{equation*}
  \frac{d}{d w} \log f(y | \mu) = - \frac{\varphi \exp(w) k_0(w | y, \varphi)}{(1 + \exp(w))^2},
\end{equation*}
where $k_0(w | y, \varphi) = \psi^{(0)}(\frac{\varphi \exp(w)}{1 + \exp(w)}) - \psi^{(0)}(\frac{\varphi}{1 + \exp(w)}) + \log( \frac{1}{y} - 1)$ and $\psi^{(0)}$ is the $0th$ derivative of the digamma function.

It can be shown that the second derivative of the beta distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \frac{\varphi \exp(2w) k_1(w | y, \varphi)}{(1 + exp(w))^4},
\end{equation*}
where $k_1(w | y, \varphi) = \varphi [\psi^{(1)}(\frac{\varphi \exp(w)}{1 + \exp(w)}) + \psi^{(1)}( \frac{\varphi}{1 + \exp(w)})] - 2\sinh(w)[k_0(w | y, \varphi) + 2\tanh^{-1}(1 - 2y)]$ and $\psi^{(n)}$ is the $nth$ derivative of the digamma function.

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = \sum_i \log(\Gamma(\varphi)) - \log(\Gamma(y_i \varphi)) - \log(\Gamma((1 - y_i)\varphi)) +  (y_i \varphi - 1) \log(y_i) + ((1 - y_i)\varphi - 1) \log(1 - y_i).
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \log(\Gamma(\varphi)) - \log(\Gamma(\hat{\mu}_i \varphi)) - \log(\Gamma((1 - \hat{\mu}_i)\varphi)) +  (\hat{\mu}_i \varphi - 1) \log(y) + ((1 - \hat{\mu}_i)\varphi - 1) \log(1 - y).
\end{equation*}

Thus the deviance is
\begin{equation*}
  2 \sum_i - \log(\Gamma(y_i \varphi)) - \log(\Gamma((1 - y_i) \varphi )) + \log(\Gamma(\hat{\mu}_i \varphi)) + \log(\Gamma((1 - \hat{\mu}_i) \varphi )) + (y_i - \hat{\mu}_i) \varphi \log(y_i) + (\hat{\mu}_i - y_i) \varphi \log(1 - y_i)
\end{equation*}
...
\end{equation*}
where $y > 0$, $\mu > 0$, $\text{E}(y) = \mu$, and $\text{Var}(y) = \mu^2/\varphi$. 

The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu) = - \log(\Gamma(\varphi)) + \varphi [\log(\varphi) - \log(\mu)] + (\varphi - 1) \log(y) - \frac{y \varphi}{\mu}.
\end{equation*}

Using the inverse log link and writing in terms of $\mu = exp(w)$, the log-likelihood can be written as
\begin{equation*}
  \log f(y | \mu) = - \log(\Gamma(\varphi)) \varphi [\log(\varphi) - w] + (\varphi - 1) \log(y) - \frac{y \varphi}{\exp(w)}.
\end{equation*}

The derivative of the gamma distribution with respect to $w$ is
\begin{equation*}
  \frac{d}{d w} \log f(y | \mu) = -\varphi + \frac{\varphi y}{\exp(w)}.
\end{equation*}

The second derivative of the gamma distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \frac{\varphi y}{\exp(w)}.
\end{equation*}

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = - \log(\Gamma(\varphi)) + \varphi [\log(\varphi) - \log(y_i)] + (\varphi - 1) \log(y_i) - \frac{y_i \varphi}{y_i}.
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \sum_i y_i - \log(\Gamma(\varphi)) + \varphi [\log(\varphi) - \log(\hat{\mu}_i)] + (\varphi - 1) \log(y_i) - \frac{y_i \varphi}{\hat{\mu}_i}.
\end{equation*}

Thus the deviance is
\begin{equation*}
  2 \sum_i - \log(\frac{y}{\hat{\mu}_i}) + \frac{y - \hat{\mu}_i}{\hat{\mu}_i}
\end{equation*}
after scaling by $\varphi$.
...
The log-likelihood (of a single observation) is defined as
\begin{equation*}
  \log f(y | \mu) = \frac{1}{2}[\log(\varphi / 2 \pi y^3)  + \log(\mu)] - \varphi \frac{(y - \mu)^2}{2 \mu y}.
\end{equation*}

Using the inverse log link and writing in terms of $\mu = exp(w)$, the log-likelihood can be written as
\begin{equation*}
  \log f(y | \mu) = \frac{1}{2}[\log(\varphi / 2 \pi y^3)  + w] - \varphi \frac{(y - \exp(w))^2}{2 \exp(w) y}.
\end{equation*}

The derivative of the gamma distribution with respect to $w$ is
\begin{equation*}
  \frac{d}{d w} \log f(y | \mu) = \varphi \left( \frac{y}{2 \exp(w)} - \frac{\exp(w)}{2y} \right) + \frac{1}{2}.
\end{equation*}

The second derivative of the gamma distribution with respect to $w$ is
\begin{equation*}
  \frac{d^2}{d w^2} \log f(y | \mu) = - \frac{\varphi(\exp(2w) + y^2)}{2y\exp(w)}.
\end{equation*}

Note that this is not a typical parameterization of the inverse Gaussian distribution. The typical parameterization of the inverse Gaussian distribution is
\begin{equation*}
 f(y | \mu, \lambda) = \sqrt{\frac{\lambda}{2 \pi y^3}} \exp \left( - \frac{\lambda(y - \mu)^2}{2u^2y} \right),
\end{equation*}
where $y > 0$, $\mu > 0$, $\text{E}(y) = \mu$, and $\text{Var}(y) = \mu^3/\lambda$, and $\lambda = \mu \varphi$. 

Twice the log-likelihood of the saturated model is 
\begin{equation*}
  2 \log f(\mathbf{y} | \boldsymbol{\mu}_s) = \frac{1}{2} \left( \frac{\lambda}{2 \pi y_i^3} \right)
\end{equation*}

Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \frac{1}{2} \left( \frac{\lambda}{2 \pi y_i^3} \right) - \frac{\lambda (y_i - \hat{\mu}_i)^2}{2 \hat{\mu}_i^2 y_i}.
\end{equation*}
...
\end{equation*}
after scaling by $\lambda$.

### Table of Inverse Link Functions, $d_i$, and $D_{i, i}$ {#sec:di_and_Di}

Table$~$\ref{tab:d_and_D} contains a table of inverse link functions, $d_i$, and $D_{i, i}$ for each spatial generalized linear model family. See more details for each family in the previous subsections.

\begin{table}
  \centering
  \begin{tabular}{cccc}
  \hline \\
  Family & $\mu = g^{-1}(w)$ & $d_i$ & $D_{i, i}$ \\
  \hline
  ...
  \hline
  \end{tabular}
  \caption{A table of inverse link functions, $d_i$, and $D_{i,i}$ for each spatial generalized linear model family.}
  \label{tab:d_and_D}
\end{table}
...
The products in this document that involve $\boldsymbol{\Sigma}^{1/2}$ and $\boldsymbol{\Sigma}^{-1}$ are actually implemented in `spmodel` using $\mathbf{C}$ and $\mathbf{C}^{-1}$. The products in this document that involve $\boldsymbol{\Sigma}^{-1/2}$ still rely on an eigendecomposition (because recall that generally, $\mathbf{C}^{-1} \mathbf{A} \neq \boldsymbol{\Sigma}^{-1/2} \mathbf{A}$). An example is computing the Pearson residuals.

# A Note on Computational Stability {#sec:computational}

Spatial covariance matrices that have approximately no independent error variance ($\sigma^2_{ie}$) can have unstable inverses. When this occurs, a small value can be added to the diagonal of the covariance matrix (via updating $\sigma^2_{ie}$) to impose some computational stability. In `spmodel`, if $\sigma^2_{ie}$ is approximately zero, a small amount is added to the diagonal of the covariance matrix. Specifically, for spatial linear models, $\sigma^2_{ie, up} = \text{max}(\sigma^2_{ie}, \sigma^2_{de}/1000)$, where $\sigma^2_{ie, up}$ denotes an "updated" version of $\sigma^2_{ie}$. For spatial generalized linear models, $\sigma^2_{ie, up} = \text{max}(\sigma^2_{ie}, \sigma^2_{de}/1000, d)$, where $d = \text{max}(1/1000, s^2/1000)$, where $s^2$ is the sample variance of a linear regression of $\log(\mathbf{y} + 1)$ on $\mathbf{X}$. This value of $\sigma^2_{ie, up}$ is also added to the diagonal of $\mathbf{X}^\top \boldsymbol{\Sigma}^{-1} \mathbf{X} + \mathbf{X}^\top \boldsymbol{\Sigma}^{-1} (\mathbf{D} - \boldsymbol{\Sigma}^{-1})^{-1} \boldsymbol{\Sigma}^{-1} \mathbf{X}$, used via the Sherman-Morrison-Woodbury formula required to efficiently invert the Hessian, $\mathbf{H}$, when using "local indexing" for big data. For more on stability of spatial covariance matrices, see @diamond1984robustness, @posa1989conditioning, @odowd1991conditioning, @ababou1994condition, @booker1999rigorous, @martin2005use, @bivand2013sp, and @ver2018kriging, among others. 
