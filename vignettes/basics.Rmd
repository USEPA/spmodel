...
We make predictions at the locations in `sulfate_preds` and store them as a new variable in the `sulfate_preds` data set called `model_preds` by running
```{r}
sulfate_preds$model_preds <- predict(sulfmod, newdata = sulfate_preds)
```

We visualize these predictions by running
```{r, eval = FALSE}
ggplot(sulfate_preds, aes(color = model_preds)) +
  geom_sf(size = 2) +
  scale_color_viridis_c(limits = c(0, 45))
```
...
```{r}
sulfate_preds$model_preds <- NULL
```
