...
We can revisit the `sulfate` data and instead model `log_Zn` as a gamma random variable. We fit a spatial generalized linear model regressing sulfate on an intercept using the `Gamma` family and a spherical spatial covariance function by running
```{r}
gsulfmod <- spglm(sulfate ~ 1, family = "Gamma", data = sulfate, spcov_type = "exponential")
```
