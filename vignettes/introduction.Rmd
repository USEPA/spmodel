...
This vignette comes with the installed version of `spmodel` (accessible via `browseVignettes(package = "spmodel")`. Eight more in-depth vignettes (i.e., articles) are available only on the `spmodel` website; we briefly describe each below.

# Website-Only Vignettes

The following vignettes (i.e., articles) are not installed alongside `spmodel` package but are available on our website at [https://usepa.github.io/spmodel/](https://usepa.github.io/spmodel/):

* [A Detailed Guide to `spmodel`](https://usepa.github.io/spmodel/articles/guide.html): A deep walkthrough of `spmodel`, covering some spatial modeling background, the available estimation methods, prediction, and advanced features like random effects, partition factors, anisotropy, and big data approaches.
* [Spatial Generalized Linear Models in `spmodel`](https://usepa.github.io/spmodel/articles/SPGLMs.html): Fitting, summarizing, and predicting with `spglm()` and `spgautor()` for binary, proportion, count, and skewed spatial data.
* [Using `emmeans` to Estimate Marginal Means of `spmodel` Objects](https://usepa.github.io/spmodel/articles/emmeans.html): Estimating and contrasting marginal means (i.e., least-squares means) for `spmodel` model objects using the `emmeans` package.
* [Block Prediction (i.e., Block Kriging) in `spmodel`](https://usepa.github.io/spmodel/articles/block.html): Predicting the average (or total) value of a response variable across a geographic region rather than at a single location.
* [Simulating Large Spatial Data in `spmodel`](https://usepa.github.io/spmodel/articles/simulation.html): Simulating spatially correlated data with `sprnorm()` and related functions, including support for large sample sizes.
* [Conditional Simulation in `spmodel`](https://usepa.github.io/spmodel/articles/conditional.html): Generating many plausible random samples of model parameters and `newdata` using `conditional()`, especially useful for quantifying uncertainty in nonlinear functions of model parameters and predictions.
* [The Spatial Decorrelation Transformation for Machine Learning Models](https://usepa.github.io/spmodel/articles/decorrelate.html): Account for spatial dependence in machine learning algorithms (e.g., random forests, gradient boosting) using `decorrelate()` and related functions.
* [Technical Details](https://usepa.github.io/spmodel/articles/technical.html): The technical details behind every relevant function in `spmodel`.
...
anova(cariboumod)
```
There seems to be significant evidence that at least one tarp cover impacts nitrogen. Note that, like in `summary()`, these p-values are associated with a hypothesis test that uses Satterthwaite denominator degrees of freedom by default when the sample size is small (here, an F-test), and an asymptotic Chi-squared test otherwise.
...
* `conditional()`: Generate many plausible random samples of model parameters and `newdata`.
* `confint()`: Compute confidence intervals.
* `cooks.distance()`: Compute Cook's distance.
* `covmatrix()`: Return covariance matrices.
* `decorrelate()`, `decorrelate_data()`, `decorrelate_newdata()`, `recorrelate_newdata()`: Account for spatial dependence in machine learning algorithms via the spatial decorrelation transformation.
* `deviance()`: Compute the deviance.
* `eacf()`: Compute an empirical autocovariance.
...
* `satterthwaite()`: Compute Satterthwaite degrees of freedom for fixed effect coefficients.
...
3. Various other **R** functions including `formula()`, `model.frame()`, `update()`, `terms()`, `car::vif()`, `car::Anova()`, and `effects::predictorEffect()`.

# Other Compatible **R** Functions

Beyond the functions listed above, `spmodel` objects implement several standard **R** generics under the hood -- `formula()`, `model.frame()`, `model.matrix()`, and `logLik()`, among others. Many general-purpose **R** and other-package functions that only rely on these pieces work on `spmodel` objects even though `spmodel` never defines a method for them directly. Confirmed examples include `update()` (refit a model with a modified formula), `terms()`, `car::vif()` and `car::Anova()` from the `car` package, and `effects::predictorEffect()` from the `effects` package. The default `AIC()` and `BIC()` also work this way: `spmodel` does not define dedicated `AIC()`/`BIC()` methods, relying instead on **R**'s own default methods, which only require a working `logLik()`.

This pattern does not extend to every generic, however. A few notable functions that do *not* currently work on `spmodel` objects include `stats::simulate()`, the base `effects()` generic (distinct from `effects::predictorEffect()`, which does work), and `extractAIC()` (and, by extension, `add1()`/`drop1()`, which depend on it).
