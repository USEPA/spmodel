...
### `loocv()`: The `"none"` and `"ie"` Simplification {#sec:loocv-iid-lm}

For `splm()` objects with covariance type `"none"` or `"ie"` and no random effects, `loocv()` uses an exact solution for predictions and standard errors. Let $e_i$ be the response residual and $h_{ii} = \mathbf{X}_i(\mathbf{X}^\top\mathbf{X})^{-1}\mathbf{X}_i^\top$ be its leverage (hat value). Then the leave-one-out residual and prediction are
\begin{equation*}
  y_i - \hat{y}_i = \frac{e_i}{1 - h_{ii}}, \qquad
  \hat{y}_i = y_i - \frac{e_i}{1 - h_{ii}}.
\end{equation*}
When the model includes an offset, $e_i$ is computed from the offset-inclusive fitted response, so this identity also returns an offset-inclusive prediction.

Writing $\hat\sigma^2_{ie}$ for the full-data independent-error variance (`none`/`ie`), the general prediction variance above simplifies to
\begin{equation*}
  \dot\sigma_i^2
  = \hat\sigma^2_{ie}\left[1 + \mathbf{X}_i(\mathbf{X}_{-i}^\top\mathbf{X}_{-i})^{-1}\mathbf{X}_i^\top\right]
  = \frac{\hat\sigma^2_{ie}}{1 - h_{ii}}.
\end{equation*}
Thus `se.fit` (the standard error used for prediction intervals) is $\sqrt{\hat\sigma^2_{ie}/(1-h_{ii})}$.
...
Options for big data cross validation rely on the `local` argument. The `local` list for `predict()` is explained in detail in the `predict()` section, but we provide a short summary of how `local` interacts with `loocv()` and `kcv()` here.

The IID `loocv()` shortcut described above is an exception to neighborhood-based approximation. Eligible `splm()` objects use this shortcut regardless of `local = FALSE`, `local = TRUE`, or a `local` list: `method` and `size` do not change their leave-one-out predictions or standard errors. A list containing `parallel = TRUE` and `ncores` can still parallelize the standard-error calculations (including those needed for prediction intervals). This shortcut accounts for re-estimating $\boldsymbol\beta$ without the held-out observation; it does not hold the full-data coefficient estimate fixed as the local approximation described below does.
