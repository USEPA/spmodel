...
`spmodel` provides support for spatial
...
least squares). @ver2024marginal and the Technical Details vignette provide the methodology's full details.
...
`spcov_type`, one of 20 spatial covariance types (e.g.,
...
| `range_constrain` | Whether to constrain the range for numerical stability | `spglm()` |
| `W` | A neighborhood weight matrix | `spgautor()`  |
| `row_st` | Whether to row-standardize `W` | `spgautor()`  |
| `M` | A symmetry matrix | `spgautor()`  |
| `range_positive` | Whether the range parameter is restricted to be positive | `spgautor()` |
| `cutoff` | Distance cutoff if using distance-based neighbor definition | `spgautor()` |
...
| `dispersion` | Dispersion assumed when computing prediction standard errors |
| `terms` | Which model terms to return when `type = "terms"` |
| `local` | Options for large data sets  |
| `var_correct` | Whether prediction variances should be adjusted |
| `delta` | Whether to return delta method standard errors on the response scale |
| `newdata_size` | The `size` value for each `newdata` observation for the binomial family |
| `na.action` | How missing values in `newdata` are handled |
...
with moose presence and that the effect of elevation varies across
the two strata (all $p~$values $<$ 0.05). 

The area under the receiver operating characteristic (AUROC) curve quantifies the effectiveness of a classifier [@robin2011proc]. It ranges from zero to one, and higher values indicate better model performance. The AUROC of `bin_spmod_anis` is
```{r}
...
```{r}
aug_mod <- augment(bin_spmod_anis, type.predict = "response", type.residuals = "response")
aug_mod
```
...
`.std.resid` are standardized residuals. By default (i.e., without
supplying `type.predict`/`type.residuals`), `.fitted` and `.resid`
are instead returned on the link scale and as deviance residuals,
respectively, matching `broom::augment.glm()`'s own defaults.
...
aug_mod <- augment(count_mod_nb, type.predict = "response")
