...
spcov_vector.none <- function(spcov_params, dist_vector) {
  ...
  # equal to column length here (it is this structure so it works with reml / ml for predict
  # and for sv-wls and sv-cl for estimation)
  # spcov_vector_val <- ifelse(dist_vector == 0, spcov_params[["ie"]], 0)
  spcov_vector_val
}
...
spcov_vector.wave <- function(spcov_params, dist_vector) {
  ...
  # spcov_vector_val <- ifelse(dist_vector == 0, spcov_params[["de"]] + spcov_params[["ie"]], spcov_vector_val)
  spcov_vector_val
}
...
#   spcov_vector_val <- spcov_params[["de"]] * 2^(1 - spcov_params[["extra"]]) / gamma(spcov_params[["extra"]]) * eta^spcov_params[["extra"]] * besselK(eta, nu = spcov_params[["extra"]])
...
#   spcov_vector_val[is.nan(spcov_vector_val) | is.infinite(spcov_vector_val)] <- spcov_params[["de"]]
#   spcov_vector_val <- ifelse(dist_vector == 0, spcov_vector_val + spcov_params[["ie"]], spcov_vector_val)
#   spcov_vector_val
# }
