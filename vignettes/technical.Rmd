...
\begin{equation*}
(\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1} + (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1} \mathbf{W} (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1},
\end{equation*}
where 
\begin{equation*}
...
\end{equation*}
where $\boldsymbol{\hat{\beta}}_i = (\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1}\mathbf{X}_i \hat{\boldsymbol{\Sigma}}^{-1}_i \mathbf{y}_i$. A similar adjustment could use $\boldsymbol{\hat{\beta}}_i = (\mathbf{X}_i^\top \hat{\boldsymbol{\Sigma}}^{-1}_{i, i} \mathbf{X}_i)^{-1}\mathbf{X}_i \hat{\boldsymbol{\Sigma}}^{-1}_i \mathbf{y}_i$, which more closely resembles a composite likelihood approach. This approach is sensitive to the presence of at least one singularity in $(\mathbf{X}_i^\top \hat{\boldsymbol{\Sigma}}_{i, i} \mathbf{X}_i)^{-1}$, in which case the variance adjustment cannot be computed.
The `"pooled"` variance adjustment is given by
\begin{equation*}
\frac{1}{m^2} \sum_{i = 1}^m (\mathbf{X}^\top_i \hat{\boldsymbol{\Sigma}}^{-1}_i \mathbf{X}_i)^{-1}.
\end{equation*}
...
# A Note on Covariance Square Roots and Inverse Products {#sec:iroot}

Talk about SVD / eigendecomposition here.
