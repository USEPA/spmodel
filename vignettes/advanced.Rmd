...
*   Spatial linear models for autoregressive data (conditional and simultaneous autoregressive spatial covariance functions).
...
`splm()` and [`spautor()`](#autoreg) (for autoregressive data) incorporate random effects using similar syntax as used by the `lme()` function from the [nlme](https://cran.r-project.org/package=nlme) R package and the `lmer()` function from the [lme4](https://cran.r-project.org/package=lme4) R package. Random effects are specified using the `random` argument, which is a formula built from variables in `data`. A couple of common ways to specify random effects in `random` include:
...
The original coordinates are first multiplied by the rotation matrix, which rotates the coordinates clockwise by angle $\alpha$. They are then multiplied by the scaling matrix, which scales the minor axis of the spatial covariance ellipse imposed by the coordinates by the reciprocal of $S$. This type of anisotropy is more formally known as "geometric" anisotropy because it involves a geometric transformation of the coordinates.
...
The `sprnorm()` function is used to simulate spatially dependent normal (Gaussian) random variables. Suppose we want to simulate at a set of 3000 locations in the unit square. First, we randomly generate 3000 x-coordinates and y-coordinates by running
```{r}
n <- 3000
x <- runif(n)
y <- runif(n)
...
We then split up the simulated data so that 90% of the observations are used for model-fitting and 10% are used for prediction.

```{r}
fit_rows <- 1:2700
sim_data_model <- sim_data[fit_rows, ]
sim_data_pred <- sim_data[-fit_rows, ]
```
...
```{r}
big1 <- splm(sim_resp ~ 1, sim_data_model, "exponential", x, y, local = TRUE)
summary(big1)
```
...
big2 <- splm(sim_resp ~ 1, sim_data_model, "exponential", x, y, local = local_list)
summary(big2)
```
...
```{r, results = "hide"}
predict(big1, sim_data_pred, local = TRUE)
```

Or we could augment the prediction data with the predictions:
```{r}
augment(big1, newdata = sim_data_pred, local = TRUE)
```

We specify a more complicated version of `local` by running
```{r, results = "hide"}
predict(big1, sim_data_pred, local = list(method = "distance", size = 30, parallel = TRUE, ncores = 2))
```
...
# # workaround using sf
# plot(seal["log_abund"], pal = hcl.colors)
```
...
# workaround using sf
# seal_preds <- augment(sealmod, newdata = sealmod$newdata)
# seal <- seal[!is.na(seal$log_abund), , drop = FALSE]
# seal_preds <- seal_preds[, -1]
# names(seal_preds)[1] <- "log_abund"
# seal <- rbind(seal, seal_preds)
# plot(seal["log_abund"], pal = hcl.colors)
```
...
Here we revisit the `caribou` data used in the basic features vignette from a spatial autoregressive perspective. Recall the first few rows of the `caribou` data:
```{r}
caribou
```
