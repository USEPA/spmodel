...
ggplot2 is only installed alongside `spmodel` when `dependencies = TRUE` in `install.packages()`, so check that it is installed before proceeding.
...
We perform an analysis of variance and tidy the reslults by running
```{r}
tidy(anova(cariboumod))
```
