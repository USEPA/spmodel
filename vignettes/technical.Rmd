...
Big data model-fitting is accommodated in `spmodel` using a "local" spatial indexing (SPIN) approach [@ver2023indexing]. Suppose there are $m$ unique indexes, and each observation is in one index. Then $\boldsymbol{\Sigma}$ can be represented blockwise as
...
`AIC()` and `AICc()` for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:aic-lm}).

## `anova()` {#sec:anova-glm}

`anova()` for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:anova-lm}).

## `coef()` {#sec:coef-glm}

`coef()` for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:coef-lm}).

## `confint()` {#sec:confint-glm}

`confint()` for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:confint-lm}).

## `cooks.distance()`  {#sec:cooks-glm}

The Cook's distance is defined as the standard generalized linear model Cook's distance after conditioning on $\mathbf{w}$. That is, after conditioning on $\mathbf{w}$, the Cook's distance is
\begin{equation}
\frac{\mathbf{e}_p^2}{p}\frac{diag(\mathbf{H}_c)}{1 - diag(\mathbf{H}_c)},
\end{equation}
where $\mathbf{e}_p^2$ are the Pearson residuals, $diag(\mathbf{H}_c)$ is the diagonal of the leverage (hat) matrix conditional on $\mathbf{w}$ and given by $\mathbf{H}_c = \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$, where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$ and $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(w_i)$.

## `deviance()` {#sec:deviance-glm}

The deviance is defined conditional on $\mathbf{w}$ (i.e., we find the deviance of the conditional model). It is derived by taking the deviance definitions from Section$~$\ref{sec:distributions-glm} and evaluating $\mu_i$ at $w_i$.
...
`pseudoR2()` for spatial generalized linear models is defined the same as for spatial linear models (Section$~$\ref{sec:pr2-lm}).
...
\end{equation*}
where $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(w_i)$.

When `type = "deviance"`, deviance residuals are returned:
\begin{equation*}
 \mathbf{e}_{d} = \text{sign}(\mathbf{e}_r) \sqrt{\text{deviance}_i},
\end{equation*}
where $\text{deviance}_i$ is the deviance of $y_i$ (conditional on $w_i$).

When `type = "standardized"`, standardized residuals are returned:
\begin{equation*}
 \mathbf{e}_{s} = \frac{\mathbf{e}_{d}}{\sqrt{1 - diag(\mathbf{H}_c)}},
\end{equation*}
where $diag(\mathbf{H}_c)$ is the diagonal of the leverage (hat) matrix conditional on $\mathbf{w}$ and given by $\mathbf{H}_c \equiv \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$, where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$.
...
\begin{equation*}
  [\mathbf{y}|\mathbf{X}, \phi, \boldsymbol{\theta}] = \int_\mathbf{w} [\mathbf{y} | g^{-1}(\mathbf{w}), \phi] [\mathbf{w} | \mathbf{X}, \boldsymbol{\theta}] d\mathbf{w} .
\end{equation*}
...
\begin{equation*}
  \int_\mathbf{w} \exp(\ell_\mathbf{w}) d\mathbf{w} \approx \exp(\ell_\mathbf{a}) \int_\mathbf{w} \exp \left(-\frac{1}{2} (\mathbf{w} - \mathbf{a})^\top (- \mathbf{H})(\mathbf{w} - \mathbf{a}) \right) d\mathbf{w} = \exp(\ell_\mathbf{a}) (2 \pi)^{n/2} |-\mathbf{H}_a|^{-1/2}, 
\end{equation*}
...
Random effects for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:random-lm}). Note that random effects for spatial generalized linear models are on the link scale.

### Anisotropy (`spglm()` only) {#sec:anisotropy-glm}

Anisotropy for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:anisotropy-lm}). Note that anisotropy parameters for spatial generalized linear models are on the link scale.

### Partition Factors {#sec:partition-glm}

Partition factors for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:partition-lm}).
...
\end{equation*}
The SMW formula is important because if the inverse and log determinant of $\mathbf{A}$ is efficient to compute, then the inverse and log determinant of the desired sum can also be efficient to compute. This is because except for $\mathbf{A}^{-1}$ and $\mathbf{|A|}$, the SMW formula only depends on $k \times k$ inverses and log determinants.
...
As previously mentioned (Section$~$\ref{sec:estimation-glm}), fitting big data spatial generalized linear models requires a doubly-iterative algorithm. This makes it far more computationally expensive than fitting big data spatial linear models.
...
Below we provide definitions associated with each of the six generalized linear models families available in `spmodel`. Note that the poisson, binomial, gamma, and inverse Gaussian distributions are members of the exponential family, and their deviance is typically expressed as twice the difference in log-likelihoods between the saturated and fitted model times the dispersion parameter (consistent with `glm()`. The negative binomial and beta distributions are members of the exponential family when the dispersion parameter ($\vartheta$) is known, and their deviance is typically expressed as just twice the difference in log-likelihoods between the saturated and fitted model (consistent with `glm.nb()` from `MASS` [@venables2002mass] for negative binomial regression and `betareg()` from `statmod` [@cribari2010beta] for beta regression). 
...
Twice the log-likelihood of the fitted (observed) model is
\begin{equation*}
  2 \log f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \log(\Gamma(\varphi)) - \log(\Gamma(\hat{\mu}_i \varphi)) - \log(\Gamma((1 - \hat{\mu}_i)\varphi)) +  (\hat{\mu}_i \varphi - 1) \log(y_i) + ((1 - \hat{\mu}_i)\varphi - 1) \log(1 - y_i).
\end{equation*}
