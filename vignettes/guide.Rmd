...
Note that more of the technical details regarding many of the functions in `spmodel` can be found in the `"technical"` vignette, which can be loaded with `vignette("technical", "spmodel")`.
...
ggplot2 is only installed alongside `spmodel` when `dependencies = TRUE` in `install.packages()`, so check that the package is installed before reproducing any visualizations in this vignette.
...
\end{equation}
where for a sample size $n$, $\mathbf{y}$ is an $n \times 1$ column vector of response variables, $\mathbf{X}$ is an $n \times p$ design (model) matrix of explanatory variables, $\boldsymbol{\beta}$ is a $p \times 1$ column vector of fixed effects controlling the impact of $\mathbf{X}$ on $\mathbf{y}$, and $\boldsymbol{\epsilon}$ is an $n \times 1$ column vector of random errors. In a simple linear regression model with independent random errors, we assume that $\text{E}(\boldsymbol{\epsilon}) = \mathbf{0}$ and $\text{Cov}(\boldsymbol{\epsilon}) = \sigma^2_\epsilon \mathbf{I}$, where $\text{E}(\cdot)$ denotes expectation, $\text{Cov}(\cdot)$ denotes covariance, $\sigma^2_\epsilon$ denotes a variance parameter, and $\mathbf{I}$ denotes the identity matrix.
...
\end{equation*}  
where $\mathbf{h}$ is a matrix of Euclidean distances among observations. Recall that $\phi$ is the range parameter, controlling the behavior of of the covariance function as a function of distance. Parameterizations for `splm()` spatial covariance types and their $\mathbf{R}$ matrices can be seen by running `help("splm", "spmodel")` or `vignette("technical", "spmodel")`.
...
<!-- MH: spcov_type is no longer required. I don't mind saying that it is required here since we want users to think about what to specify it as, but this requirement statement is not actually true anymore. -->
...
moss
```
We can learn more about `moss` by running `help("moss", "spmodel")`, and we can visualize the distribution of log zinc concentration in the `moss` data (Figure$~$\ref{fig:log_zn}) with 
...
<!-- MH: Is it a mistake that the rotate and scale parameters still print out with `coef()`? -->

```{r}
coef(spmod, type = "spcov")
```

<!-- MH: consider cutting discussion of effective range here -->

<!-- MH: consider cutting discussion of anisotropy here -->
...
<!-- MH: check added definition of \Theta. You do define it when discussing AIC in the next section but it should be defined here. -->
...
\end{equation*}
where $\boldsymbol{\hat{\Theta}}$ is the estimated parameter vector that includes both the estimated fixed effect vector $\boldsymbol{\hat{beta}}$ and the estimated spatial covariance parameter vector $\boldsymbol{\hat{\theta}}$, $\mathcal{D}(\boldsymbol{\hat{\Theta}})$ is the deviance of the fitted model indexed by parameter vector $\boldsymbol{\hat{\Theta}}$, and $\mathcal{D}(\boldsymbol{\hat{\Theta}}_0)$ is the deviance of an intercept-only model indexed by parameter vector $\boldsymbol{\hat{\Theta}}_0$. We compute the pseudo R-squared by running
```{r}
pseudoR2(spmod)
```
...
In addition to model fit metrics, `spmodel` also provides functions to compute diagnostics metrics to help assess model assumptions and identify "unusual" observations.
...
<!-- MH: added word inverse. check it's okay.  -->
...
<!-- MH: Though not incorrect, the discussion of leverage calculations uses the theoretical \Sigma while the pearson residuals calculations use \hat{\Sigma} and are framed as what spmodel will actually return. This is especially problematic in defining e_s, which uses e_p (and therefore \hat{Sigma}) as well as H* (which uses \Sigma). -->
...
<!-- MH: Consider omitting visualization of Cook's distance vs. leverage -->
...
The `tidy()`, `glance()`, and `augment()` functions from the broom \textbf{\textsf{R}} package [@robinson2021broom] provide convenient output for many of the summary metrics and diagnostics metrics discussed in the previous two sections. The `tidy()` function returns a tidy tibble of the coefficient table from `summary()`:
```{r}
tidy(spmod)
```
...
Next, we use the areal `seal` data, an `sf` object that contains the log of the estimated harbor-seal trends from abundance data across polygons in Alaska, to give an example of fitting a model with `spautor()`. We view the first few rows of `seal` by running
```{r}
seal
```
We can learn more about the data by running `help("seal", "spmodel")`.

To visualize the distribution of log seal trends in the `seal` data (Figure$~$\ref{fig:log_trend}), we can run
...
Polygons are typically considered neighbors if they share at least one boundary. The gray polygons denote areas where the log trend is missing. These missing areas need to be kept in the data while fitting the model to preserve the overall neighborhood structure. 
...
```
By default, `spautor()` calculates the weight matrix internally by defining observations as neighbors if they share at least one border (observations are not neighbors with themselves), uses row standardization, and assumes an independent error variance of zero.
...
<!-- MH: consider omitting. If kept, revise so that it doesn't just read like a long list of relatively disconnected thoughts. -->
...
<!-- MH: Is removing the predicted values and doing this binding too confusing for a person reading this initially? One solution would be to include sulfate binded with sulfate_preds as a data set in spmodel (sulfate_full perhaps) and then using this directly. -->

Alternatively, we can include missing responses (`NA` values) in the data used for model fitting. The missing responses will be ignored for model fitting but stored in the fitted model object. We can add a column of missing responses to `sulfate_preds`, remove the fitted values we used to make Figure$~$\ref{fig:sulfate}, bind it together with `sulfate`, and re-fit the model by running
```{r}
