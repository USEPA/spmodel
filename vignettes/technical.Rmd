...
\end{equation}
where $\mathbf{H}_c = \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$ is the leverage (hat) matrix conditional on $\mathbf{w}$, $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$ and $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(\text{w}_i)$.

## `deviance()` {#sec:deviance-glm}

The deviance is of the fitted model conditional on the $\mathbf{w}$ (i.e., the deviance of the conditional model). Thus the deviance (of the conditional model) is derived by taking the deviance definitions from Section$~$\ref{sec:distributions-glm} and evaluating $\mu_i$ at w$_i$.
...
When `type = "standardized"`, standardized residuals are returned:
\begin{equation*}
 \mathbf{e}_{s} = \frac{\mathbf{e}_{p}}{\sqrt{1 - diag(\mathbf{H}_c)}},
\end{equation*}
where $diag(\mathbf{H}_c)$ is the diagonal of the hat matrix, $\mathbf{H}_c \equiv \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$, where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$.
...
As with `splm()` and `spautor()` for spatial linear models, many of the details of `spglm()` and `spgautor()` for spatial generalized linear models are the same, though occasional differences are noted in the following subsection headers.
...
\end{equation*}
The term $[y|g^{-1}(\mathbf{w}), \phi]$ is likelihood of the generalized linear model with mean function $g^{-1}(\mathbf{w})$, and the term $[\mathbf{w}|\mathbf{X}, \boldsymbol{\theta}]$ is the restricted log likelihood for $\mathbf{w}$ given the covariance parameters. 
...
\begin{equation*}
  \int_\mathbf{w} \exp(\ell_\mathbf{w}) d\mathbf{w} \approx exp(\ell_\mathbf{a}) \int_w \exp(\frac{1}{2} (\mathbf{w} - \mathbf{a})^\top (- \mathbf{H})(\mathbf{w} - \mathbf{a})) d\mathbf{w} = exp(\ell_\mathbf{a}) (2 \pi)^{n/2} |-\mathbf{H}_a|^{-1/2}|, 
\end{equation*}
...
\begin{equation*}
  -2\ell_p(\phi, \boldsymbol{\theta}) = -2\ln([\mathbf{y} | g^{-1}(\mathbf{a}), \phi]) -2\ln([\mathbf{a} | \mathbf{X}, \boldsymbol{\theta}]) + \ln(|- \mathbf{H}_{\mathbf{a}}(\phi, \boldsymbol{\theta})|).
\end{equation*}
...
\end{equation*}
where $D_{i, i} = \partial^2 \ln[y_i | g^{-1}(w_i), \phi]  / \partial w_i^2$. Note that $D_{i, j} = 0$ for $i \neq j$ because of conditional independence. Table$~$\ref{tab:d_and_D}. So Newton-Rhapson can be used to maximizing $\ell_{\mathbf{w}}$, where an update is given by
\begin{equation*}
...
Anisotropy for spatial generalized linear models are defined the same as for spatial linear models (Section$~$\ref{sec:anisotropy-glm}). Note that anisotropy parameters for spatial generalized linear models correspond to variance and range parameters on the link scale.
...
Big data model-fitting for spatial generalized linear models in many ways the same as for spatial linear models (Section$~$\ref{sec:random-glm}). The `local` argument behaves the same for spatial generalized linear models as it is for spatial linear models. This is because fundamentally, the "local indexing" approach to representing $\boldsymbol{\Sigma}$ blockwise is still applied and serves as the basis for massive computational gains. 
