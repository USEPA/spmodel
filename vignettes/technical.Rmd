...
### `splmRF()` and `spautorRF()` {#sec:rf_pred}

Random forest spatial residual model (Section$~$\ref{sec:rf}) predictions are obtained by combining random forest predictions and spatial linear model predictions (i.e., Kriging) of the random forest residuals. Formally, 
...
\end{equation*}
where $\mathbf{e}_{rf}$ are the random forest residuals. Random forest spatial residual models can significantly improve predictive accuracy for new data compared to standard random forest models by formally incorporating spatial covariance in the random forest residuals [@fox2020comparing]. Random forest spatial residual model prediction is discussed in Section$~$\ref{sec:rf_pred}.
...
## Distribution Parameteriziations
