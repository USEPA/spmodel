...
\begin{equation*}
  \begin{split}
   ...
   p & = \text{The number of fixed effects} \\
   \mathbf{X} & = \text{Design matrix of predictor variables (covariates)} \\
   \mathbf{Z} & = \text{Design matrix of random effect variables} \\
   ...
   \mathbf{a}^* & = \Sigma^{-1/2}\mathbf{a} \text{ for a general vector } \mathbf{a} 
  \end{split}
\end{equation*}
...
\begin{equation*}
  \begin{split}
   ...
   \mathbf{X}_o & = \text{Design matrix of observed predictor variables} \\
   \mathbf{X}_u & = \text{Design matrix of unobserved predictor variables} \\
...
\begin{equation*}\label{eq:sp_aic}
  ...
  \end{split}
\end{equation*}
where $|\hat{\boldsymbol{\Theta}}|$ is the cardinality of $\hat{\boldsymbol{\Theta}}$.  For restricted maximum likelihood, $\hat{\boldsymbol{\Theta}} \equiv \{\hat{\boldsymbol{\theta}}\}$. For maximum likelihood, $\hat{\boldsymbol{\Theta}} \equiv \{\hat{\boldsymbol{\theta}}, \hat{\boldsymbol{\beta}}\}$ The discrepancy arises because restricted maximum likelihood integrates the fixed effects out of the likelihood. Thus, the likelihood does not depend on $\boldsymbol{\beta}$, though it does still depend on $\mathbf{X}$.
...
\begin{equation*}\label{eq:glht}
  X^2 = [(\mathbf{L} \boldsymbol{\hat{\beta}} - l_0)^\intercal(\mathbf{L} (\mathbf{X}^\intercal \mathbf{\hat{\Sigma}} \mathbf{X})^{-1} \mathbf{L}^\intercal)^{-1}(\mathbf{L} \boldsymbol{\hat{\beta}} - l_0)]
\end{equation*}
...
\begin{equation*}
\hat{\beta}_i \pm z^* \sqrt{(\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1}_{ii}},
\end{equation*}
...
Cook's distance measures the influence of an observation [@cook1979influential]. An influential observation has a large impact on the model fit. The Cook's distance for the $i$th observation is given by 
\begin{equation} \label{eq:cooksd}
D_i = \frac{\hat{\mathbf{e}}_i}{p}\frac{h^*_{ii}}{1 - h^*_{ii}},
\end{equation}
where $\hat{\mathbf{e}}_i$ is the Pearson (whitened) residual for the $i$th observation and $h^*_{ii}$ is the $i$th diagonal entry of the whitened hat matrix, $\mathbf{H}^*$ [@montgomery2021introduction]. A suggested cutoff for identifying influential observations is 1 [@cook1982residuals].

To better understand the equation in Equation \ref{eq:cooksd}, recall that the the non-spatial linear model $\mathbf{y} = \mathbf{X} \boldsymbol{\beta} + \boldsymbol{\epsilon}$ assumes elements of $\boldsymbol{\epsilon}$ are independent and identically distributed (iid) with constant variance. For this model, the Cook's distance for the $i$th observation is given by $\frac{\hat{\mathbf{e}}_i}{p}\frac{h_{ii}}{(1 - h_{ii})}$, where $\mathbf{H} = \mathbf{X} (\mathbf{X}^\intercal \mathbf{X})^{-1} \mathbf{X}^\intercal$ is the hat matrix. When the elements of $\boldsymbol{\epsilon}$ are not iid or do not have constant variance or both, the Cook's distance cannot be calculated using $\mathbf{H}$ directly. First the linear model must be whitened according to $\mathbf{y}^* = \mathbf{X}^* \boldsymbol{\beta} + \boldsymbol{\epsilon}^*$, where $\boldsymbol{\epsilon}^*$ is the whitened version of the sum of all random errors in the model. Then the Cook's distance follows using $\mathbf{H}^*$, where $\mathbf{H}^* = \mathbf{X}^* (\mathbf{X}^{* \intercal} \mathbf{X})^{-1} \mathbf{X}^{* \intercal}$.
...
\end{equation*}
where $\ell(\boldsymbol{\Theta}_s)$ is twice the log-likelihood of a "saturated" model that fits every observation perfectly. For normal (Gaussian) random errors,
\begin{equation*}
\mathcal{D}_{\boldsymbol{\Theta}} = (\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})^\intercal \hat{\boldsymbol{\Sigma}}^{-1} (\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})
\end{equation*}
...
\begin{equation*}\label{eq:blup_mm}
  \mathbf{\hat{u}} = \sigma^2_u \mathbf{Z}^\intercal \mathbf{\Sigma}^{-1}(\mathbf{y} - \mathbf{X} \boldsymbol{\hat{\beta}}).
\end{equation*}
...
Hat values measure the leverage of an observation. An observation has high leverage if its combination of predictor variables is atypical (far from the mean predictor vector). The leverage (hat value) of the $i$th observation is given by
\begin{equation}
\label{eq:leverage}
 h^*_{ii} = (\mathbf{X}^* (\mathbf{X}^{* \intercal} \mathbf{X})^{-1} \mathbf{X}^{* \intercal})_{ii},
\end{equation}
where $\mathbf{H}^*_{ii}$ is the $i$th diagonal entry of the whitened hat matrix, $\mathbf{H}^*$ [@montgomery2021introduction]. One suggested cutoff for identifying observations with high leverage is $3p / n$.

To better understand the equation in Equation \ref{eq:leverage}, recall that the the regular linear model $\mathbf{y} = \mathbf{X} \boldsymbol{\beta} + \boldsymbol{\epsilon}$ assumes elements of $\boldsymbol{\epsilon}$ are iid with constant variance. For this model, the leverage for the $i$th observation is given by $\mathbf{H}_{ii}$, where $\mathbf{H} = \mathbf{X} (\mathbf{X}^\intercal \mathbf{X})^{-1} \mathbf{X}^\intercal$ is called the hat matrix. When the elements of $\boldsymbol{\epsilon}$ are not iid or do not have constant variance or both, leverage cannot be calculated using $\mathbf{H}$ directly. First, the linear model must be whitened according to $\mathbf{y}^* = \mathbf{X}^* \boldsymbol{\beta} + \boldsymbol{\epsilon}^*$, where $\boldsymbol{\epsilon}^*$ is the whitened version of the sum of all random errors in the model. Then the leverage follows using $\boldsymbol{\epsilon}^*$ and $\mathbf{H}^*$, where $\mathbf{H}^* = \mathbf{X}^* (\mathbf{X}^{* \intercal} \mathbf{X})^{-1} \mathbf{X}^{* \intercal}$.
...
\begin{equation*}
 \boldsymbol{\Sigma}^{-1} = 
 \begin{bmatrix}
  \tilde{\boldsymbol{\Sigma}}_{-i, -i} & \tilde{\boldsymbol{\Sigma}}_{i,-i}^\intercal \\
...
\begin{equation*}
 \boldsymbol{\Sigma}^{-1}_{-i, -i} = \tilde{\boldsymbol{\Sigma}}_{-i, -i} - \tilde{\boldsymbol{\Sigma}}_{i,-i}^\intercal \tilde{\boldsymbol{\Sigma}}_{i, i}^{-1}\tilde{\boldsymbol{\Sigma}}_{i,-i}
\end{equation*}
and 
\begin{equation*}
  \boldsymbol{\beta}_{-i} = (\mathbf{X}^\intercal_{-i} \boldsymbol{\Sigma}^{-1}_{-i, -i} \mathbf{X}_{-i})^{-1} \mathbf{X}^\intercal_{-i} \boldsymbol{\Sigma}^{-1}_{-i, -i} \mathbf{y}_{-i},
\end{equation*}
where $\mathbf{X}^\intercal_{-i}$ is $\mathbf{X}$ with the $i$th row deleted and $\mathbf{y}_{-i}$ is $\mathbf{y}$ with the $i$th element deleted. The prediction of the $i$th observation is then evaluated at $\boldsymbol{\beta}_{-i}$ instead of $\boldsymbol{\beta}$. Model fits are evaluated using mean squared prediction error (mspe), formally defined as
\begin{equation*}
...
\begin{equation}\label{eq:blup_cov}
  \dot{\boldsymbol{\Sigma}}_u = \hat{\boldsymbol{\Sigma}}_u - \hat{\boldsymbol{\Sigma}}_{uo} \hat{\boldsymbol{\Sigma}}^{-1}_o \hat{\boldsymbol{\Sigma}}^\intercal_{uo} + \mathbf{Q}(\mathbf{X}_o^\intercal \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X}_o)^{-1}\mathbf{Q}^\intercal ,
\end{equation}
...
The best linear unbiased estimates of $\text{E}[(y_u)_i]$ ($\text{E}(\cdot)$ denotes expectation) are returned by evaluating $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}}$ (i.e., fitted values corresponding to $(\mathbf{X}_u)_i)$. The (100 $\times$ `level`)% confidence interval for $\text{E}[(y_u)_i]$ is $(\mathbf{X}_u)_i \hat{\boldsymbol{\beta}} \pm z^* \sqrt{(\mathbf{X}_u)_i (\mathbf{X}^\intercal_o \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X}_o)^{-1} (\mathbf{X}_u)_i^\intercal}$, where $(\mathbf{X}_u)_i$ is the $i$th row of $\mathbf{X}_u$, $\sqrt{(\mathbf{X}_u)_i (\mathbf{X}^\intercal_o \hat{\boldsymbol{\Sigma}}_o^{-1} \mathbf{X}_o)^{-1} (\mathbf{X}_u)_i^\intercal}$ is the standard error of $(\dot{y}_u)_i$ obtained from `se.fit = TRUE`, $\Phi(z^*) = 1 - \alpha / 2$, $\Phi(\cdot)$ is the standard normal (Gaussian) cumulative distribution function, $\alpha = 1 -$ `level`, and `level` is an argument to `predict()`. The default for `level` is 0.95, which corresponds to a $z^*$ of approximately 1.96.
...
\begin{equation*}\label{eq:auto_hw}
  \boldsymbol{\Sigma}^{-1} =
  \begin{bmatrix}
    \tilde{\boldsymbol{\Sigma}}_{o} & \tilde{\boldsymbol{\Sigma}}^{\intercal}_{uo} \\
...
\begin{equation*}\label{eq:hw_forms}
  \begin{split}
    \boldsymbol{\Sigma}^{-1}_o & = \tilde{\boldsymbol{\Sigma}}_{o} - \tilde{\boldsymbol{\Sigma}}^{ \intercal}_{uo} (\tilde{\boldsymbol{\Sigma}}_{u})^{-1} \tilde{\boldsymbol{\Sigma}}_{uo} \\
    \boldsymbol{\Sigma}_u & = (\tilde{\boldsymbol{\Sigma}}_{u} - \tilde{\boldsymbol{\Sigma}}_{uo} (\tilde{\boldsymbol{\Sigma}}_{o})^{-1} \tilde{\boldsymbol{\Sigma}}^\intercal_{uo})^{-1} \\
...
\begin{equation*}
PR2 = 1 - \frac{(\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})^\intercal \hat{\boldsymbol{\Sigma}}^{-1}(\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})}{(\mathbf{y} - \hat{\boldsymbol{\mu}})^\intercal \hat{\boldsymbol{\Sigma}}^{-1}(\mathbf{y} - \hat{\boldsymbol{\mu}})},
\end{equation*}
where $\hat{\mu} = (\boldsymbol{1}^\intercal \hat{\boldsymbol{\Sigma}}^{-1} \boldsymbol{1})^{-1} \boldsymbol{1}^\intercal \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{y}$. For the non-spatial model, the pseudo R-squared reduces to the classical R-squared, as
\begin{equation*}
PR2 = 1 - \frac{(\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})^\intercal \hat{\boldsymbol{\Sigma}}^{-1}(\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})}{(\mathbf{y} - \hat{\boldsymbol{\mu}})^\intercal \hat{\boldsymbol{\Sigma}}^{-1}(\mathbf{y} - \hat{\boldsymbol{\mu}})}  = 1 - \frac{(\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})^\intercal (\mathbf{y} - \mathbf{X} \hat{\boldsymbol{\beta}})}{(\mathbf{y} - \hat{\boldsymbol{\mu}})^\intercal (\mathbf{y} - \hat{\boldsymbol{\mu}})} = 1 - \frac{\text{SSE}}{\text{SST}} = R2,
\end{equation*}
...
\begin{equation*}
  \begin{split}
  \text{Cov}(\hat{\boldsymbol{\Sigma}}^{-1/2} \mathbf{e}_{r}) & \approx \boldsymbol{\Sigma}^{-1/2} \text{Cov}(\mathbf{e}_{r}) \boldsymbol{\Sigma}^{-1/2 \intercal} \\
  & \approx \boldsymbol{\Sigma}^{-1/2} \boldsymbol{\Sigma} \boldsymbol{\Sigma}^{-1/2 \intercal} \\
  & = \boldsymbol{\Sigma}^{-1/2} \boldsymbol{\Sigma}^{1/2} \boldsymbol{\Sigma}^{1/2 \intercal} \boldsymbol{\Sigma}^{-1/2 \intercal} \\
  & = (\mathbf{I}) (\mathbf{I}) \\
  & = \mathbf{I}
  \end{split}
\end{equation*}
...
\begin{equation*}
  ...
  & = (\mathbf{I} - \mathbf{H}^*) \mathbf{I} (\mathbf{I} - \mathbf{H}^*)^\intercal \\
  & = (\mathbf{I} - \mathbf{H}^*),
  \end{split}
\end{equation*}
...
\end{equation*}
where $\sigma^2_{de}$ is the spatially dependent (correlated) variance for the connected observations, $\mathbf{R}$ is the spatial correlation matrix for the connected observations, $\sigma^2_{\xi}$ is the independent (not correlated) variance for the unconnected observations, and $\sigma^2_{ie}$ is the independent (not correlated) variance for all observations. As seen, the connected and unconnected observations are allowed different variances. The total variance for connected observations is then $\sigma^2_{de} + \sigma^2_{ie}$ and the total variance for unconnected observations is $\sigma^2_{\xi} + \sigma^2_{ie}$. spmodel accommodates two forms for $\mathbf{R}$: conditional autoregressive (CAR) and simultaneous autoregressive (SAR). For both CAR and SAR covariance functions, $\mathbf{R}$ depends on similar quantities: $\mathbf{I}$, an identity matrix; $\phi$, a range parameter, and $\mathbf{W}$, a matrix that defines the neighborhood structure. Often $\mathbf{W}$ is symmetric but it need not be. Valid values for $\phi$ are in $(1 / \lambda_{max}, 1 / \lambda_{min})$, where $\lambda_{min}$ is the minimum eigenvalue of $\mathbf{W}$ and $\lambda_{max}$ is the maximum eigenvalue of $\mathbf{W}$. For SAR covariance functions, $\lambda_{min}$ must be negative and $\lambda_{max}$ must be positive. For CAR covariances functions, a matrix $\mathbf{M}$ matrix must be provided that satisfies the CAR symmetry condition, which enforces the symmetry of the covariance matrix. The CAR symmetry condition states 
\begin{equation*}
...
The default in spmodel is to row-standardize $\mathbf{W}$ by dividing each element by its respective row sum, which decreases variance. If row-standardization is not used for a CAR model, the default in spmodel for $\mathbf{M}$ is the identity matrix. Table$~$\ref{tab:cov_spautor} shows the parametric form of $\mathbf{R}$ for CAR and SAR covariance functions.

\begin{table}\label{tab:cov_spautor}
  \centering
  \begin{tabular}{c|l}
  \hline
  Spatial covariance type Name & $\mathbf{R}$ functional form \\
  \hline
  \texttt{"car"} & $(\mathbf{I} - \phi\mathbf{W})^{-1}\mathbf{M}$ \\
  \texttt{"sar"} & $[(\mathbf{I} - \phi\mathbf{W})(\mathbf{I} - \phi\mathbf{W})^\intercal]^{-1}$ \\
  \hline
  \end{tabular}
  \caption{Table of spatial covariance functions types available in \texttt{spautor()}.}
\end{table}
...
\end{equation*}
where $\sigma^2_{de}$ is the spatially dependent (correlated) random error, $\mathbf{R}$ is a spatial correlation matrix, $\sigma^2_{ie}$ is the spatially independent (not correlated) random error, and $\mathbf{I}$ is an identity matrix. The $\mathbf{R}$ matrix always depends on a range parameter that controls the behavior of the covariance function with distance. For some covariance functions, the $\mathbf{R}$ matrix depends on an additional parameter that we call the "extra" parameter.  Table$~$\ref{tab:cov_splm} shows the parametric form for all $\mathbf{R}$ matrices available in `splm()`. In Table$~$\ref{tab:cov_splm}, the range parameter is denoted as $\phi$, the distance divided by the range parameter ($h / \phi$) is denoted as $\eta$, $\mathbbm{1}\{\cdot\}$ is an indicator function equal to 1 when $\cdot$ occurs and 0 otherwise, and the extra parameter is denoted as $\xi$ (when relevant).
\begin{table}\label{tab:cov_splm}
  \centering
  \begin{tabular}{c|l}
  \hline
  Spatial covariance type Name & $\mathbf{R}$ functional form \\
  ...
  \texttt{"cauchy"} & $(1 + \eta^2)^{-\xi}$ \\
  \texttt{"pexonential"} & $exp(-h^\xi / \phi)$ \\
  \texttt{"none"} & $0$ \\
  \hline
  \end{tabular}
  \caption{Table of spatial covariance functions types available in \texttt{splm()}. All spatial covariance functions are valid in two dimensions except \texttt{"triangular"} and \texttt{"cosine"}, which are only valid in one dimension.}
\end{table}
...
\begin{equation*}\label{eq:ml-plik}
  -2\ell_p^*(\boldsymbol{\theta}^*) = \ln{|\boldsymbol{\Sigma^*}|} + n\ln[(\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})^\intercal \boldsymbol{\Sigma}^{* -1} (\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})] + n + n\ln{2\pi / n}.
\end{equation*}
After finding $\hat{\boldsymbol{\theta}}^*_{ml}$, a closed form solution for $\hat{\sigma}^2_{ml}$ exists: $\hat{\sigma}^2_{ml} = [(\mathbf{y} - \mathbf{X} \boldsymbol{\tilde{\beta}})^\intercal \mathbf{\Sigma}^{^{* -1}} (\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})] / n$. Then $\boldsymbol{\hat{\theta}}^*_{ml}$ is combined with $\hat{\sigma}^2_{ml}$ to yield $\boldsymbol{\hat{\theta}}_{ml}$ and subsequently $\boldsymbol{\hat{\beta}}_{ml}$. A similar result holds for restricted maximum likelihood estimation. Profiling $\sigma^2$ out of Equation$~$\ref{eq:reml-lik} yields
\begin{equation*}\label{eq:reml-plik}
  -2\ell_R^*(\boldsymbol{\Theta}) = \ln{|\boldsymbol{\Sigma^*}|} + (n - p)\ln[(\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})^\intercal \boldsymbol{\Sigma}^{* -1} (\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})] + \ln{|\mathbf{X}^\intercal \boldsymbol{\Sigma}^{* -1} \mathbf{X}|} + (n - p) + (n - p)\ln2\pi / (n - p).
\end{equation*}
After finding $\hat{\boldsymbol{\theta}}^*_{reml}$, a closed form solution for $\hat{\sigma}^2_{reml}$ exists: $\hat{\sigma}^2_{reml} = [(\mathbf{y} - \mathbf{X} \boldsymbol{\tilde{\beta}})^\intercal \mathbf{\Sigma}^{^{* -1}} (\mathbf{y} - \mathbf{X} \tilde{\boldsymbol{\beta}})] / (n - p)$. Then $\boldsymbol{\hat{\theta}}^*_{reml}$ is combined with $\hat{\sigma}^2_{reml}$ to yield $\boldsymbol{\hat{\theta}}_{reml}$ and subsequently $\boldsymbol{\hat{\beta}}_{reml}$. For more on profiling Gaussian likelihoods, see @wolfinger1994computing.
...
Parameter estimation is performed using `stats::optim()`. The default estimation method is Nelder-Mead [@nelder1965simplex] and the stopping criterion is a relative convergence tolerance (`reltol`) of .0001. If only one parameter requires estimation (on the profiled scale if relevant), the Brent algorithm is instead used [@brent1971algorithm]. Arguments to `optim()` are passed via `...` to `splm()` and `spautor()`. For example, the default estimation method and convergence criteria are overridden by passing `method` and `control`, respectively, to `splm()` and `spautor()`. If the `lower` and `upper` arguments to `optim()` are passed to `splm()` and `spautor()`, they are ignored, as optimization for all parameters is generally unconstrained. Initial values for `optim()` are found using the grid search described next.

### Grid Search

spmodel uses a grid search to find suitable initial values for use in optimization. For spatial linear models without random effects, the `de`, `ie` parameters are given "low", "medium", and "high" values. The sample variance of a non-spatial linear model is slightly inflated by a factor of 1.2 (non-spatial models can underestimate the variance when there is spatial dependence) and these "low", "medium", and "high" values correspond to 10%, 50%, and 90% of the inflated sample variance. Only combinations of `de` and `ie` whose proportions sum to 100% are considered. The `range` and `extra` parameters are given "low" and "high" values that are unique to each spatial covariance function. The `rotate` parameter is given six values that correspond to 0, $\pi/6$, $2\pi/6$, $4\pi/6$, $5\pi/6$, and $\pi$ radians. The `scale` parameter is given "low", "medium", and "high" values that correspond to scaling factors of 0.25, 0.75, and 1. Note that the `rotate` and `scale` parameters are only used during grid searches for point-referenced data. 
...
params <- data.frame(
  de = c(9, 1, 5, 9, 1, 5),
  ie = c(1, 9, 5, 1, 9, 5),
  range = c(15, 15, 15, 45, 45, 45),
  rotate = c(0, 0, 0, 0, 0, 0),
  scale = c(1, 1, 1, 1, 1, 1)
)
knitr::kable(params, caption = "\\label{tab:grid1}Grid search parameter configurations for an isotropic exponential spatial covariance with inflated sample variance 10.")
```
...
```{r grid2, echo = FALSE}
sp_dom <- data.frame(
  de = c(8.1, 0.9, 4.5, 8.1, 0.9, 4.5),
  ie = c(0.9, 8.1, 4.5, 0.9, 8.1, 4.5),
  range = c(15, 15, 15, 45, 45, 45),
  rotate = c(0, 0, 0, 0, 0, 0),
  scale = c(1, 1, 1, 1, 1, 1),
  rand1 = c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5),
  rand2 = c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5)
)
...
knitr::kable(rbind(sp_dom, rand_dom, even_dom), caption = "\\label{tab:grid2}Grid search parameter configurations for an isotropic exponential spatial covariance with two random effects and inflated sample variance 10.")
```
...
\begin{equation*}
(\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}} \mathbf{X})^{-1} + (\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}} \mathbf{X})^{-1} \mathbf{W} (\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}} \mathbf{X})^{-1},
\end{equation*}
where 
\begin{equation*}
\mathbf{W} = \sum_{i = 1}^{m - 1} \sum_{j = i + 1}^m [(\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}}^{-1}_{i, i} \hat{\boldsymbol{\Sigma}}_{i, j} \hat{\boldsymbol{\Sigma}}^{-1}_{j, j} \mathbf{X}_j) + (\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}}^{-1}_{i, i} \hat{\boldsymbol{\Sigma}}_{i, j} \hat{\boldsymbol{\Sigma}}^{-1}_{j, j} \mathbf{X}_j)^\intercal]
\end{equation*}
...
\begin{equation*}
\frac{1}{m(m -1)} \sum_{i = 1}^m (\boldsymbol{hat{\beta}}_i - \boldsymbol{\hat{\beta}})(\boldsymbol{\hat{\beta}}_i - \boldsymbol{\hat{\beta}})^\intercal,
\end{equation*}
where $\boldsymbol{\hat{\beta}}_i = (\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}} \mathbf{X})^{-1}\mathbf{X}_i \hat{\boldsymbol{\Sigma}}_i \mathbf{y}_i$. A similar adjustment could use $\boldsymbol{\hat{\beta}}_i = (\mathbf{X}_i^\intercal \hat{\boldsymbol{\Sigma}}_{i, i} \mathbf{X}_i)^{-1}\mathbf{X}_i \hat{\boldsymbol{\Sigma}}_i \mathbf{y}_i$, which more closely resembles a composite likelihood approach. This approach is sensitive to the presence of at least one singularity in $(\mathbf{X}_i^\intercal \hat{\boldsymbol{\Sigma}}_{i, i} \mathbf{X}_i)^{-1}$, in which case the variance adjustment cannot be computed.
The `"pooled"` variance adjustment is given by
\begin{equation*}
\frac{1}{m^2} \sum_{i = 1}^m (\mathbf{X}^\intercal_i \hat{\boldsymbol{\Sigma}}_i \mathbf{X}_i)^{-1}.
\end{equation*}
Note that the pooled variance adjustment cannot be computed if any $(\mathbf{X}_i^\intercal \boldsymbol{\Sigma}_i^{-1} \mathbf{X}_i)^{-1}$ are singular.
...
`vcov()` returns the variance-covariance matrix of estimated parameters. Currently, `vcov()` only returns the variance-covariance matrix of $\hat{\boldsymbol{\beta}}$, the fixed effects. The variance-covariance matrix of the fixed effects is given by $(\mathbf{X}^\intercal \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1}$.
