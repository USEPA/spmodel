...
\end{equation*}
where $g(\cdot)$ is called a link function, $\boldsymbol{\mu}$ is the mean of $\mathbf{y}$, and the remaining terms $\mathbf{X}$, $\boldsymbol{\beta}$, $\boldsymbol{\tau}$, $\boldsymbol{\epsilon}$ represent the same quantities as for the spatial linear models (Section$~$\ref{sec:splm}). The link function, $g(\cdot)$, "links" a function of $\boldsymbol{\mu}$ to the linear term $\boldsymbol{\eta}$, denoted here as $\mathbf{X} \boldsymbol{\beta} + \boldsymbol{\tau} + \boldsymbol{\epsilon}$, which is familiar from spatial linear models. Each link function $g(\cdot)$ has a corresponding inverse link function, $g^{-1}(\cdot)$. The inverse link function "links" a function of $\boldsymbol{\eta}$ to $\boldsymbol{\mu}$ (and applies element-wise to each vector).  Notice that for spatial generalized linear models, we are not modeling $\mathbf{y}$ directly as we do for spatial linear models, but rather we are modeling a function of the mean of $\mathbf{y}$. Also notice that $\boldsymbol{\eta}$ is unconstrained but $\boldsymbol{\mu}$ is usually constrained in some way (e.g., positive). Next we discuss the specific distributions modeled and link functions used in `spmodel`.
...
Printing `poismod` shows the function call, the estimated fixed effect coefficients (on the link scale), the estimated spatial covariance parameters, and a dispersion parameter. The dispersion parameter is estimated in some models and changes the mean-variance relationship of $\mathbf{y}$. For Poisson regression models, the dispersion parameter is not estimated and always fixed at one.
```{r}
print(poismod)
```
...
The `tidy()`, `glance()`, and `augment()` functions behave similarly for `spglm()` objects as they did for `splm()` objects. 
...
We glance at the spatial Poisson regression model and a non-spatial Poisson regression model by running
```{r}
glmod <- spglm(count ~ elev, family = "poisson", data = moose, spcov_type = "none")
glances(poismod, glmod)
```

The much lower AIC and AICc for the spatial Poisson regression model indicates it is a much better fit to the data.
...
We can revisit the `sulfate` data and instead model `sulfate` as a gamma random variable. We fit a spatial generalized linear model regressing sulfate on an intercept using the `Gamma` family and a spherical spatial covariance function by running
```{r}
gsulfmod <- spglm(sulfate ~ 1, family = "Gamma", data = sulfate, spcov_type = "spherical")
```
...
We tidy the fixed effect (on the link scale) by running
```{r}
tidy(gsulfmod)
```
...
Note that the model fit statistics returned by `glance()` (e.g., AIC) for a model fit using `splm()` should not be compared to those from a model fit using `spglm()`, as the models evaluate different likelihoods. 
...
We augment the data with predictions of each observation's mean (on the link scale) by running
```{r}
augment(gsulfmod, newdata = sulfate_preds)
```

In this case, predictions from the spatial linear model and predictions of each observation's mean (on the inverse link scale) from the spatial gamma model are similar:
```{r}
# splm predictions
sulfate_preds$splm <- predict(sulfmod, newdata = sulfate_preds)
ggplot(sulfate_preds, aes(color = splm)) +
  geom_sf(size = 2) +
  scale_color_viridis_c(limits = c(0, 45))

# spglm predictions (on the response scale)
sulfate_preds$spglm <- predict(gsulfmod, newdata = sulfate_preds, type = "response")
ggplot(sulfate_preds, aes(color = spglm)) +
  geom_sf(size = 2) +
  scale_color_viridis_c(limits = c(0, 45))
```

```{r, echo = FALSE}
sulfate_preds$splm <- NULL
sulfate_preds$spglm <- NULL
```
...
by running `?generic.spmodel` (e.g., `?fitted.spmodel`, `?predict.spmodel`, `?summary.spmodel`, etc.).
