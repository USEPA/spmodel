...
\end{equation*} where $\mathbf{y}$ is a vector of responses, $\mathbf{X}$ is a matrix of explanatory variables, $\boldsymbol{\beta}$ is a vector of fixed effects, $\mathbf{Z}_i$ is a matrix that describes the $i$th random effect, $u_i$, $\boldsymbol{\tau}$ is a vector of spatially dependent (correlated) random errors, and $\boldsymbol{\epsilon}$ is a vector of spatially independent (not correlated) random errors.
...
We glance at the models fit without (`basic`) and with (`rand1`) random intercepts by running
```{r}
glances(basic, rand1)
```

The model with the random effect has a much lower AIC, indicating it is a better fit to the data. A similar conclusion is made when performing a likelihood ratio test that comapres the full model with a random intercepts for `sample` to the reduced model (which is nested in the full model) without the random intercepts for `sample`:
```{r}
...
We tidy random effects by running
```{r}
tidy(rand1, effects = "randcov")
```

We obtain best linear unbiased predictions (BLUPs) of random effects (similar to `ranef()` from nlme or lme4) by running
```{r, results = "hide"}
...
The simplest way to implement the default big data approximation is to set the `local` argument equal to `TRUE`. This is shorthand for `local = list(method = "random", size = 50, var_adjust = "theoretical", parallel = FALSE)`:
```{r}
local1 <- splm(sim_resp ~ 1, sim_data, "exponential", x, y, local = TRUE)
summary(local1)
```
...
local2 <- splm(sim_resp ~ 1, sim_data, "exponential", x, y, local = local2_list)
summary(local2)
```
...
```{r, results = "hide"}
predict(local1, sim_preds, local = TRUE)
```

Or we could augment the prediction data with the predictions:
```{r}
augment(local1, newdata = sim_preds, local = TRUE)
```
...
sim_preds$preds <- predict(local1, sim_preds, local = TRUE)
...
```{r, results = "hide"}
predict(local1, sim_preds, local = list(method = "distance", size = 30, parallel = TRUE, ncores = 2))
```
...
We visualize the distribution of the log seal trends by running
```{r}
plot(
  seal["log_trend"],
  pal = hcl.colors,
...
We fit a spatial autoregressive model of the log seal trends on an intercept using a conditional autoregressive (CAR) spatial covariance by running
```{r}
...
We previously mentioned that for spatial autoregressive models, prediction can only be performed for missing-data polygons that were incorporated into model fitting. Thus, prediction using `spautor()` does not accommodate "new" locations like prediction using `splm()` does. We predict the log seal trends at the 28 sites in `seal` with missing-data polygons by running `predict(sealmod)`. Recall that the missing-data polygons are stored as `newdata` in `seal`:
```{r}
sealmod$newdata
```

We augment `newdata` by running
```{r}
...
seal2 <- seal[!is.na(seal$log_trend), , drop = FALSE]
seal_preds2 <- seal_preds2[, -1]
names(seal_preds2)[1] <- "log_trend"
...
# ggplot() +
#   geom_sf(data = seal, aes(fill = log_trend)) + 
...
While the fitted model in this example only uses an intercept, the same code is used for prediction with fitted models having explanatory variables.
