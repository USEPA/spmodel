...
`confint()` returns confidence intervals for estimated parameters. Currently, `confint()` only returns confidence intervals for $\boldsymbol{\beta}$. For `splm()`/`spautor()` model objects fit with `ddf = "asymptotic"` (or for `spglm()`/`spgautor()` model objects, which have no `ddf` argument), the $(1 - \alpha)$% confidence interval for $\beta_i$ is
\begin{equation*}
...
For `splm()`/`spautor()` model objects fit with `ddf = "satterthwaite"` (the default when $n \le 500$; see `satterthwaite()`), $z^*$ is instead replaced by $t^*$, the $(1 - \alpha/2)$ quantile of a $t$-distribution with $\nu_i$ (the $i$th fixed effect's Satterthwaite denominator degrees of freedom) degrees of freedom: $\hat{\beta}_i \pm t^* \sqrt{(\mathbf{X}^\top \hat{\boldsymbol{\Sigma}}^{-1} \mathbf{X})^{-1}_{i, i}}$. Because a $t$-distribution has heavier tails than a standard normal distribution, Satterthwaite confidence intervals are wider than asymptotic confidence intervals, appropriately reflecting the extra uncertainty from estimating $\boldsymbol{\theta}$ in a finite sample.
...
\end{equation*} The prediction $\hat{\mathbf{y}}_F$ and prediction covariance matrix $\dot{\boldsymbol{\Sigma}}_F$ (not just a prediction variance, since the $m$ held-out predictions in a fold are correlated with one another) follow by substituting $F$ for $i$ throughout the single-observation loocv equations above -- e.g. $\dot{\boldsymbol{\Sigma}}_F = \hat{\boldsymbol{\Sigma}}_{F, F} - \hat{\boldsymbol{\Sigma}}_{F, -F}\hat{\boldsymbol{\Sigma}}_{-F, -F}^{-1}\hat{\boldsymbol{\Sigma}}_{F, -F}^\top + \mathbf{Q}_F(\mathbf{X}_{-F}^\top \hat{\boldsymbol{\Sigma}}_{-F, -F}^{-1}\mathbf{X}_{-F})^{-1}\mathbf{Q}_F^\top$, with $\mathbf{Q}_F = \mathbf{X}_F - \hat{\boldsymbol{\Sigma}}_{F, -F}\hat{\boldsymbol{\Sigma}}_{-F, -F}^{-1}\mathbf{X}_{-F}$; only the diagonal of $\dot{\boldsymbol{\Sigma}}_F$ (the per-observation prediction variance, matching `loocv()`'s output shape) is returned by default. At $m = 1$ ($k = n$), every quantity above reduces exactly to `loocv()`'s corresponding single-observation formula. Fold assignment is either random (`k` approximately-equally-sized folds) or user-supplied (`folds_index`), and bias, MSPE, RMSPE, and cor2 are computed identically to `loocv()`, aggregating the prediction error across every observation regardless of which fold it fell in.
...
\end{equation*}
 confirming $\dot{y}_u = \mathbf{c}_0^\top \mathbf{y}_o = \mathbf{x}_0^\top\hat{\boldsymbol{\beta}} + \hat{\boldsymbol{\Sigma}}_{uo}\hat{\boldsymbol{\Sigma}}_o^{-1}(\mathbf{y}_o - \mathbf{X}_o\hat{\boldsymbol{\beta}})$, matching the BLUP given previously.
...
\end{equation*}
where $diag(\mathbf{H}^*)$ is the diagonal of the spatial hat matrix, $\mathbf{H}_s \equiv \mathbf{X}^* (\mathbf{X}^{* \top} \mathbf{X}^*)^{-1} \mathbf{X}^{* \top}$, and $\odot$ denotes the Hadamard (element-wise) product. This residual transformation "standardizes" the Pearson residuals. As such, the standardized residuals should also have mean zero and variance \begin{equation*}
  \begin{split}
...
- `method = "closed"`: both quantities use closed-form derivatives $\partial \boldsymbol{\Sigma}/\partial \theta_j$ instead. The expected (Fisher) information is $I_{jk} = \frac{1}{2}\text{tr}(\mathbf{P}\frac{\partial\boldsymbol{\Sigma}}{\partial\theta_j}\mathbf{P}\frac{\partial\boldsymbol{\Sigma}}{\partial\theta_k})$, where $\mathbf{P} = \boldsymbol{\Sigma}^{-1}$ for maximum likelihood and $\mathbf{P} = \boldsymbol{\Sigma}^{-1} - \boldsymbol{\Sigma}^{-1}\mathbf{X}(\mathbf{X}^\top\boldsymbol{\Sigma}^{-1}\mathbf{X})^{-1}\mathbf{X}^\top\boldsymbol{\Sigma}^{-1}$ for restricted maximum likelihood (accounting for the marginalization of $\boldsymbol{\beta}$), and $\widehat{\text{Var}}(\boldsymbol{\hat\theta}) \approx \mathbf{I}^{-1}$. $\nabla g(\boldsymbol{\hat\theta})$ is obtained analytically from the same $\partial\boldsymbol{\Sigma}/\partial\theta_j$ derivatives. Closed-form derivatives are only implemented for the exponential, Gaussian, spherical, and none, and ie (equivalent to none for linear models) spatial covariance functions without anisotropy; every other covariance type or an anisotropic fit falls back to `method = "numeric"`. The default is `"closed"` when available and `"numeric"` otherwise.
...
\end{equation} where $p$ equals the dimension of $\boldsymbol{\beta}$. Minimizing \eqref{eq:reml-lik} yields $\boldsymbol{\hat{\theta}}_{reml}$, the restricted maximum likelihood estimates for $\boldsymbol{\theta}$. Then a closed for solution exists for $\boldsymbol{\hat{\beta}}_{reml}$, the restricted maximum likelihood estimates for $\boldsymbol{\beta}$: $\boldsymbol{\hat{\beta}}_{reml} = \tilde{\boldsymbol{\beta}}_{reml}$, where $\tilde{\boldsymbol{\beta}}_{reml}$ is $\tilde{\boldsymbol{\beta}}$ evaluated at $\boldsymbol{\hat{\theta}}_{reml}$.
...
\end{equation*} where $\odot$ indicates the Hadamard product. Partition factors impose a block structure in $\boldsymbol{\Sigma}$, which allows for efficient computation of $\boldsymbol{\Sigma}^{-1}$ used for estimation and prediction.
...
\mathbf{W} = \sum_{i = 1}^{m - 1} \sum_{j = i + 1}^m (\mathbf{X}_i^\top \hat{\boldsymbol{\Sigma}}^{-1}_{i, i} \hat{\boldsymbol{\Sigma}}_{i, j} \hat{\boldsymbol{\Sigma}}^{-1}_{j, j} \mathbf{X}_j) + (\mathbf{X}_i^\top \hat{\boldsymbol{\Sigma}}^{-1}_{i, i} \hat{\boldsymbol{\Sigma}}_{i, j} \hat{\boldsymbol{\Sigma}}^{-1}_{j, j} \mathbf{X}_j)^\top
\end{equation*} 
...
`varcomp()` decomposes the total variability in the response into a portion attributable to the fixed effects and a portion attributable to each variance parameter in the fitted covariance structure. The fixed effects are credited with the pseudo r-squared (`pseudoR2()`), and the remaining $1 - PR2$ is split among $\sigma^2_{de}$, $\sigma^2_{ie}$, and any random effect variances $\sigma^2_{u_k}$ in proportion to their share of the total variance:
\begin{equation*}
  \begin{split}
  ...
  \end{split}
\end{equation*}
For `spautor()` model objects with unconnected sites (i.e., a nonzero $\xi$; see `spautor()` Spatial Covariance Functions), this decomposition is instead computed separately for the connected and unconnected sites, with $\xi$ taking the place of $\sigma^2_{de}$ and $\sigma^2_{ie}$ in the unconnected sites' decomposition (unconnected sites have no spatial dependence structure to attribute variance to). A third quantity, the ratio of the connected sites' total variance to the unconnected sites' total variance, is also returned in this case.
...
\end{equation} where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$ and $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to the family's variance function evaluated at $\hat{\mathbf{w}}$.
...
\end{equation*} where $\mathbf{V}$ is a diagonal matrix with $i$th diagonal element equal to $\text{Var}(y_i)$, the (dispersion-adjusted) response-scale variance implied by the family evaluated at $\hat{\mathbf{w}}$. Note this $\mathbf{V}$ is different from the one used by `hatvalues()`, which is not adjusted for dispersion).
...
\end{equation*} where $\text{deviance}_i$ is the deviance of $y_i$ (conditional on $w_i$) and $\odot$ denotes the Hadamard (element-wise) product.
...
\end{equation*} where $diag(\mathbf{H}_c)$ is the diagonal of the leverage (hat) matrix conditional on $\mathbf{w}$ and given by $\mathbf{H}_c \equiv \mathbf{X}_v (\mathbf{X}_v^\top\mathbf{X}_v)^{-1} \mathbf{X}_v^\top$, where $\mathbf{X}_v = \mathbf{V}^{1/2}\mathbf{X}$, and $\odot$ denotes the Hadamard (element-wise) product.
...
Next the Newton-Raphson algorithm can be used to maximize $\ell_{\mathbf{w}}$, where an update is given by \begin{equation*}
 \mathbf{w}^{k + 1} = \mathbf{w}^k - \alpha \mathbf{G}^{-1}\mathbf{g}.
\end{equation*} where $0 < \alpha \leq 1$. Typically, $\alpha = 1$, but it can be lowered if the $\mathbf{g}$ is unstable. Generally, the Newton-Raphson converges rapidly. The value of $\mathbf{w}$ at convergence is defined as $\mathbf{a}$.
...
Below we provide definitions associated with each of the six generalized linear models families available in `spmodel`. Note that the poisson, binomial, gamma, and inverse Gaussian distributions are members of the exponential family, and their deviance is typically expressed as twice the difference in log-likelihoods between the saturated and fitted model divided by the dispersion parameter (consistent with `glm()`). The negative binomial and beta distributions are members of the exponential family when the dispersion parameter ($\varphi$) is known, and their deviance is typically expressed as just twice the difference in log-likelihoods between the saturated and fitted model (consistent with `glm.nb()` from `MASS` [@venables2002mass] for negative binomial regression and `betareg()` from `betareg` [@cribari2010beta] for beta regression). A table of inverse link functions and the $d_i$ and $D_{i,i}$ quantities derived below for all six families is also provided for reference at the end of this section.
...
Thus the deviance is \begin{equation*}
  2 \sum_i \left[y_i \log(y_i / \hat{\mu}_i) - (y_i - \hat{\mu}_i)\right].
\end{equation*}
...
The log-likelihood of the saturated model is \begin{equation*}
  \ln f(\mathbf{y} | \boldsymbol{\mu}_s) = \sum_i \frac{1}{2} \ln\left( \frac{\lambda}{2 \pi y_i^3} \right)
\end{equation*}

The log-likelihood of the fitted (observed) model is \begin{equation*}
  \ln f(\mathbf{y} | \hat{\boldsymbol{\mu}}) = \sum_i \frac{1}{2} \ln\left( \frac{\lambda}{2 \pi y_i^3} \right) - \frac{\lambda (y_i - \hat{\mu}_i)^2}{2 \hat{\mu}_i^2 y_i}.
\end{equation*}
...
| Family | $u = g^{-1}(w)$ | Quantity | Value |
|:-----------------|:---------------------|:------------|:--------------------------------------------------|
| Poisson | $\mu = \exp(w)$ | $d_i$ | $y_i - \exp(w_i)$ |
| | | $D_{i,i}$ | $- \exp(w_i)$ |
| Negative Binomial | $\mu = \exp(w)$ | $d_i$ | $\frac{\varphi (y_i - \exp(w_i))}{\exp(w_i) + \varphi}$ |
| | | $D_{i,i}$ | $- \frac{\varphi \exp(w_i) (y_i + \varphi)}{(\varphi + \exp(w_i))^2}$ |
| Binomial | $\mu = \frac{\exp(w)}{1 + \exp(w)}$ | $d_i$ | $y_i - \frac{m_i \exp(w_i)}{1 + \exp(w_i)}$ |
| | | $D_{i,i}$ | $- \frac{m_i \exp(w_i)}{(1 + \exp(w_i))^2}$ |
| Beta | $\mu = \frac{\exp(w)}{1 + \exp(w)}$ | $d_i$ | $- \frac{\varphi \exp(w_i) k_0(w_i \mid y_i, \varphi)}{(1 + \exp(w_i))^2}$ |
| | | $D_{i,i}$ | $- \frac{\varphi \exp(2w_i) k_1(w_i \mid y_i, \varphi)}{(1 + \exp(w_i))^4}$ |
| Gamma | $\mu = \exp(w)$ | $d_i$ | $-\varphi + \frac{\varphi y_i}{\exp(w_i)}$ |
| | | $D_{i,i}$ | $- \frac{\varphi y_i}{\exp(w_i)}$ |
| Inverse Gaussian | $\mu = \exp(w)$ | $d_i$ | $\varphi \left( \frac{y_i}{2 \exp(w_i)} - \frac{\exp(w_i)}{2y_i} \right) + \frac{1}{2}$ |
...
`predict()` (`predict()`, `predict()`) returns a single best linear unbiased prediction (and its uncertainty) at each new location. `conditional()` instead draws `samples` full realizations of $\mathbf{y}_u$ (or, for `spglm()`, $\mathbf{w}_u$) from its predictive distribution, jointly across every location in `newdata`, conditional on the observed data. This is useful whenever a nonlinear function of the entire predicted surface (e.g., a quantile, the proportion of `newdata` locations exceeding a threshold) is of interest, as the marginal prediction intervals of `interval = "prediction"` are not sufficient because they do not quantify how predictions at different locations vary together (i.e., co-vary).
...
2. Simulate the predictions for `newdata` conditional on the observed data, using $\hat{\boldsymbol{\beta}}_b$ (not $\hat{\boldsymbol{\beta}}$) to form prediction.
...
\end{equation} where $\hat{\boldsymbol{\Sigma}}_u^*$ is the same conditional covariance as \eqref{eq:cond_splm}, and $\Lambda(-\mathbf{G}_{\mathbf{a}})^{-1}\Lambda^\top$ is the same `var_correct = TRUE` correction term from \eqref{eq:glm_blup_cov}, reusing $\Lambda$ exactly as defined there. Since $\Lambda\mathbf{a} = \dot{\mathbf{w}}_u$ (the point prediction), this correction is the extra predictive variance that comes from $\mathbf{a}$ itself being an estimate of $\mathbf{w}_o$ rather than a direct observation. Consistently, $\hat{\boldsymbol{\beta}}_b$ is drawn from the full, `var_correct = TRUE` $\text{Var}(\hat{\boldsymbol{\beta}}) = \mathbf{B}(-\mathbf{G}_{\mathbf{a}})^{-1}\mathbf{B}^\top + (\mathbf{X}^\top\hat{\boldsymbol{\Sigma}}_o^{-1}\mathbf{X})^{-1}$, not the naive (uncorrected) $(\mathbf{X}^\top\hat{\boldsymbol{\Sigma}}_o^{-1}\mathbf{X})^{-1}$ alone. Omitting this correction would understate the condition simulation variances. `simulate_covparams` is not available for `spglm()` model objects.
...
\end{equation}
where often, $\hat{y} = \bar{y}$, the sample mean of all the observations. Like with `esv()`, the default is to use 15 equally spaced bins with a distance cutoff of half the maximum distance (hypotenuse) of the domain's bounding box. Because we are typically interested in characterizing residual autocovariance, $y_i - \hat{y}$ and $y_j - \hat{y}$ tend to represent residuals from an ordinary least squares fit given some fixed effects design matrix, $\mathbf{X}$, and fixed effects, $\hat{\boldsymbol{\beta}}$.
