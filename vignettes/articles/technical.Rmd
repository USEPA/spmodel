...
\end{equation*}
where $\mathbf{s}$ and $\mathbf{u}$ represent points in the region (the product of $\mathbf{s}$ and $\mathbf{u}$ contains all possible pairs of points in the region). Intuitively, $\sigma^{2*}_B$ is the average covariance between any two points in the region, approximated by summation over the fine grid:
\begin{equation*}
  \sigma^{2*}_B \approx \frac{1}{|B|^2}\sum_{g_i = 1}^G \sum_{g_j = 1}^G \text{Cov}(\text{y}_{g_i}, \text{y}_{g_j}) .
\end{equation*}
