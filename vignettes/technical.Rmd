...
\end{equation}
where $\hat{\mathbf{e}}_p$ are the Pearson residuals and $diag(\mathbf{H}_s)$ is the diagonal of the spatial hat matrix, $\mathbf{H}_s \equiv \mathbf{X}^* (\mathbf{X}^{* \top} \mathbf{X}^*)^{-1} \mathbf{X}^{* \top}$ [@montgomery2021introduction]. A suggested cutoff for identifying influential observations is 1 [@cook1982residuals].
