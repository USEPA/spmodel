...
ggplot2 is only installed alongside `spmodel` when `dependencies = TRUE` in `install.packages()`, so check that the package is installed before reproducing any of these vignette's visualizations.
...
In addition to model fit metrics, `spmodel` provides functions that compute diagnostic metrics which help assess model assumptions and identify unusual observations.
...
A partition factor is a variable that allows observations to be uncorrelated when they are from different levels of the partition factor. Partition factors are specified in `spmodel` by providing a formula with a single variable to the `partition_factor` argument. Suppose that for the `moss` data, we would like to assume observations in different years (`year`) are uncorrelated. We fit a model that treats year as a partition factor by running
```{r}
part <- splm(
  log_Zn ~ log_dist2road,
  moss,
...
<!-- MH: This is a little confusing to me, as the first sentence makes it seem like `rotate` is how much you rotate the ellipse, with a starting point as the ellipse's major axis as the x-axis. But then after reading the rest of the paragraph, I think `rotate` is how much you rotate the ellipse so that the major axis is back to the x-axis. I think the first sentence just needs revised but I am not confident enough with this to change it myself. -->
...
<!-- MH: Delete maybe? or reword: I don't think Though is quite the right word to start with. -->
<!-- Though the sample sizes used in the next subsection for model fitting and prediction are relatively small, the purpose of the following examples is simply to illustrate the big data approaches. -->
...
<!-- MH: Maybe add a quick statement about why such an indexing strategy results in faster model fitting? -->
<!-- MH: maybe delete "This approach has some connections to composite likelihood and to the partition factors discussed earlier. " -->
...
The simplest way to accommodate big data is to set `local` to `TRUE`. This is shorthand for `local = list(method = "random", size = 50, var_adjust = "theoretical", parallel = FALSE)`, which creates groups with approximately 50 observations each, uses a random assignment of groups to indexes, uses the theoretically-correct variance adjustment, and does not use parallel processing. 
```{r}
...
<!-- MH: maybe cut the loocv() section. Or, if not cut, add a sentence explaining what the purpose of looking at loocv() is for these big data approximations. -->

After we fit the models, we can use `loocv()` to evaluate model performance. Note that `loocv()` has big data options that are passed to `predict()`, which we discuss in Section$~$\ref{sec:predict}.
```{r}
...
<!-- MH: maybe get rid of predict(sealmod, ....), with the reason that it feels very out of place to jump to this different example so quickly, and people can figure out the arguments based on the previous example). -->

# Discussion {#sec:discussion}

<!-- MH: maybe cut or maybe just shorten? "Several model-fit statistics and diagnostics are available. The broom functions `tidy()` and `glance()` are used to tidy and glance at a fitted model. The broom function `augment()` is used to augment `data` with model diagnostics and augment `newdata` with predictions. Several advanced features are available to accommodate fixed covariance parameter values, random effects, partition factors, anisotropy, simulated data, and big data approximations for model fitting and prediction." -->
...
The purpose of this example is to show more examples of the `splm()` and `spautor()` functions, as well as a few other functions of the `spmodel` package. The `caribou` data are observed on an equally spaced lattice and can be analyzed as point-referenced or areal data. We view the first few rows of `caribou` by running
```{r}
caribou
```
