...
As with spatial linear models, spatial generalized linear models are fit in `spmodel` for point-referenced and areal data. The `spglm()` function is used to fit spatial generalized linear models for point-referenced data, and the `spgautor()` function is used to fit spatial generalized linear models for areal data. `spglm()` and `sugautor()` share similar syntax with `splm()` and `spautor()`, respectively, though one additional argument is required:

* `family`: The generalized linear model family (i.e., the distribution of $\mathbf{y}$). Can be `binomial`, `beta`, `Poisson`, `nbinomial`, `Gamma`, or `inverse.gaussian`.
...
Next we show the basic features and syntax of `spglm()` using the `moose` data. We study the impact of elevation (`elev`) on the total number of moose (`count`) observed at a site location in Alaska. We view the first few rows of the `moose` data by running 
```{r}
moose
```

We can visualize the distribution of moose counts (`count`) by running
```{r moose, fig.align="center", out.width = "65%", fig.cap = "Distribution of moose counts in the moose data."}
...
One example of a generalized linear model is a Poisson regression model. Poisson regression models are often used to model count data such as this. To quantify the relationship between counts and elevation, we fit a spatial Poisson regression model (a specific spatial generalized linear model) by running
```{r}
...
Similar to summaries of `glm()` objects, summaries of `spglm()` objects include the original function call, deviance residuals, and a coefficients table of fixed effects. The log of the mean counts does not appear to be related to elevation, as evidenced by the large p-value associated with the asymptotic z-test. A pseudo r-squared is also returned, which quantifies the proportion of variability explained by the fixed effects. The spatial covariance parameters and deviance parameter are also returned. The dispersion parameter is estimated in some models and changes mean-variance relationship of $\mathbf{y}$. For Poisson regression models, the dispersion parameter is not estimated and always fixed at one.
...
For spatial generalized linear models, we are predicting the mean of the process generating the observation rather than the observation itself. We make predictions of mean counts at the locations in `moose_preds` by running
```{r}
moose_preds$preds <- predict(poismod, newdata = moose_preds, type = "response")
```
...
```{r moose-pred, fig.align="center", out.width = "65%", fig.cap = "Distribution of mean moose predictions in Alaska."}
...
These predictions have similar spatial patterns as in the observed counts, as many of the observed counts were zero near locations requiring prediction. Next we remove the model predictions from `moose_preds` and show how `augment()` can be used to obtain the same predictions alongside prediction intervals (on the link scale):
```{r}
moose_preds$preds <- NULL
augment(poismod, newdata = moose_preds, type = "response", interval = "prediction")
```
...
There is noticeable patterning in the response variable (`sim_response`) in the same places where there is noticeable patterning for the spatial Gaussian data. This is expected, as the spatial Gaussian data is used as the to evaluate the mean (proportion) parameter of the binomial data. Thus where the mean parameter is higher, the more likely the binomial observation is a success.

### Additional Examples

We previously showed how to use `spmodel` to model count data via spatial Poisson regression. Recall that `spmodel` can also be used to model count data via spatial negative binomial regression, proportion data via spatial beta regression, binary data via binomial regression, and skewed, positive data via spatial gamma or spatial inverse Gaussian regression. Next we provide examples of fitting a few of these models.

Poisson regression assumes the mean and variance are equal. Often in practice, the variance is larger than the mean. When the variance is larger than the mean, there is said to be overdispersion in the data. Negative binomial regression accounts for overdispersion via an extra dispersion parameter that allows the variance to change separately from the mean (recall the full negative binomial regression parameterization is available in the technical details vignette). We fit a spatial negative binomial regression model to the moose count data by running
```{r, eval = FALSE}
nbmod <- spglm(count ~ elev, family = "nbinomial", data = moose, spcov_type = "exponential")
```
Running `glances(poismod, nbmod)` shows that the AIC and AICc are lower for the negative binomial model than the Poisson model, suggesting the negative binomial model is a better fit to the data and by consequence, there is evidence of overdispersion in the moose counts.

Binomial (Bernoulli) regression is used to model binary data (successes and failures). The `presence` variable in the `moose` data is equal to 1 (success) if there is at least one moose observed and 0 (failure) otherwise. To quantify the relationship between moose presence and elevation, we fit a spatial binomial regression model by running
```{r}
binmod <- spglm(presence ~ elev, family = "binomial", data = moose, spcov_type = "exponential")
```

Gamma regression is used to model skewed, positive data. Suppose instead of modeling the `sulfate` data using a spatial linear model, we modeled sulfate using spatial gamma regression. We fit a spatial gamma regression model to these data by running
```{r}
...
Throughout this vignette, we have shown how to use `spmodel` to fit, summarize, and predict for a variety of spatial statistical models. Spatial linear models for point-referenced data (i.e., geostatistical models) are fit using the `splm()` function while spatial linear models for areal data (i.e., spatial autoregressive models) are fit using the `spautor()` function. Several model-fit statistics and diagnostics are available. The broom functions `tidy()` and `glance()` are used to tidy and glance at a fitted model. The broom function `augment()` is used to augment `data` with model diagnostics and augment `newdata` with predictions. Several advanced features are available to accommodate fixed covariance parameter values, random effects, partition factors, anisotropy, simulated data, and big data approximations for model fitting and prediction.
