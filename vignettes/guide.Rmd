...
Next we use the areal `seal` data, an `sf` object that contains data on harbor seal trends in southeast Alaska, USA. We view the first few rows of `seal` by running
```{r}
seal
```
...
We estimate parameters of a spatial autoregressive model regressing log seal abundance (`log_abund`) on an intercept using a conditional autoregressive (CAR) spatial covariance by running
```{r}
